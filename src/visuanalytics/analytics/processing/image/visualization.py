...
def generate_all_images(values: dict, step_data: StepData, tests=False):
    """
...
@register_image
def pillow(values: dict, prev_paths: dict, presets: dict, step_data: StepData, tests=False):
    """
    ...
    file = resources.new_temp_resource_path(step_data.data["_pipe_id"], "png", tests)
    Image.composite(img1, source_img, img1).save(file)
    return file


@register_image
def wordcloud(image: dict, prev_paths, presets: dict, step_data: StepData, tests=False):
    """
