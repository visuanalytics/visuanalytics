...
out.append(ws.get_oneday_icons_image(weather.data_icon_oneday(data, 0), date[0]))
out.append(ws.get_oneday_temp_image(weather.data_temp_oneday(data, 0), date[0]))

out.append(ws.get_oneday_icons_image(weather.data_icon_oneday(data, 1), date[1]))
out.append(ws.get_oneday_temp_image(weather.data_temp_oneday(data, 1), date[1]))

out.append(ws.get_threeday_image(weather.data_icon_threeday(data), weather.data_mm_temp_threeday(data), date[2:5]))

audios = speech.first_weatherforecast_text_to_speech(get_data.add_data_together(data))
audiol = audio.get_audio_length(audios)
videolinker.to_forecast_germany(out, audios, audiol)
