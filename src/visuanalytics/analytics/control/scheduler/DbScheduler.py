...
class DbScheduler(Scheduler):
    def __run_jobs(self, job_id, is_job):
        if is_job:
            ...
            datasource_name, json_file_name, config = job.get_datasource_run_info(str(job_id))
            logger.info(f"Datasource {job_id}: '{datasource_name}' started")
            self._start_datasource(job_id, datasource_name, json_file_name, config, True)
    ...
    def _check_all(self, now: datetime):
        ...
        datasources = job.get_datasource_schedules()
