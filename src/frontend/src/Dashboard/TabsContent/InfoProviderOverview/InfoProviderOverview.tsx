...
export const InfoProviderOverview: React.FC = () => {
   ...
   */
  const handleErrorFetchAll = React.useCallback(
    (err: Error) => {
      //console.log('error');
   ...
   */
  const deleteInfoProvider = useCallFetch(
    "visuanalytics/infoprovider/" + currentDeleteId,
    {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json\n",
  ...
  const handleSuccessEdit = (jsonData: any) => {
    ...
    components?.setCurrent("editInfoProvider", {
      infoProvId: currentEditId,
  ...
  const editInfoProvider = useCallFetch(
    "/visuanalytics/infoprovider/" + currentEditId,
    {
      method: "GET",
    },
    handleSuccessEdit,
    ...
    >
      ...
      >
        <Grid item container xs={12}>
          <Grid item xs={6}>
            <Typography variant={"h5"}>Angelegte Info-Provider:</Typography>
          </Grid>
