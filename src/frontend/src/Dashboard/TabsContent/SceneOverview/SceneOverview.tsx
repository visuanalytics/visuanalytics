...
export const SceneOverview: React.FC<SceneOverviewProps> = (props) => {
  ...
  const [previewImgList, setPreviewImgList] = React.useState<
    Array<PreviewImage>
  ...
  const [currentScene, setCurrentScene] = React.useState<BackendScene>(
    {} as BackendScene
  );
  ...
  const handleFetchInfoProviderSuccess = (jsonData: any, scene: FullScene) => {
    ...
    components?.setCurrent("sceneEditor", {
      sceneFromBackend: {
        ...scene,
  ...
  const fetchSceneById = useCallFetch(
    "visuanalytics/scene/" + currentScene.scene_id,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json\n",
    ...
    handleFetchSceneSuccess,
    handleErrorFetchSceneError
  );
  ...
  const deleteSceneInBackend = useCallFetch(
    "visuanalytics/scene/" + currentScene.scene_id,
    {
      method: "DELETE",
      headers: {
  ...
  return (
    ...
    >
      ...
      >
        <Grid item xs={4}>
          <Typography variant={"h5"}>Angelegte Szenen:</Typography>
        </Grid>
        ...
        <Grid item container xs={12} justify={"center"}>
          <SceneList
            ...
            setCurrent={(data: BackendScene, index: number) =>
