...
export const LogDialog: React.FC<LogDialogProps> = (props) => {
   ...
   */
  const handleFetchLogsError = React.useCallback(
    (err: Error) => {
  ...
  const renderTableRow = (entry: LogEntry, index: number) => {
    return (
      <TableRow key={index}>
        ...
        <TableCell className={classes.logMessageTableCell}>
          {entry.errorMsg}
        </TableCell>
  ...
  return (
    <React.Fragment>
          ...
          <Grid container>
            <Grid item xs={12}>
              <Typography variant="body1" className={classes.wrappedText}>
                Hier können Sie die Log-Daten für den{" "}
                {props.objectType === "infoprovider"
                  ? "Infoprovider"
                  : "Videojob"}{" "}
                "{props.objectName}" einsehen.
              </Typography>
            </Grid>
              ...
              {logMessages.length > 0 && (
                <TableContainer component={Paper}>
                  <Table aria-label="Log-Einträge">
                    ...
                    <TableBody>
                      {logMessages.map((logEntry: LogEntry, index) =>
                        renderTableRow(logEntry, index + 1)
                      )}
                    </TableBody>
                  </Table>
                </TableContainer>
      ...
      <Dialog
        maxWidth={"md"}
        aria-labelledby="TracebackDialog-Title"
        open={showTracebackDialog}
        onClose={() => closeTracebackDialog()}
      >
        <DialogTitle id="TracebackDialog-Title">Traceback-Anzeige</DialogTitle>
        <DialogContent dividers>
          <Grid container>
            <Grid item xs={12}>
              <Typography variant="body1">{selectedTraceback}</Typography>
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions className={classes.elementLargeMargin}>
