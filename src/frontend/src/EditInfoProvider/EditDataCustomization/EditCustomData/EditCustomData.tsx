...
export const EditCustomData: React.FC<EditCustomDataProps> = (props) => {
   ...
   */
  const [currentEditFormel, setCurrentEditFormel] = React.useState(
    new FormelObj("", "", [])
  );
  ...
  const deleteCustomDataCheck = (formelName: string) => {
    ...
    props.infoProvDataSources[props.selectedDataSource].customData.forEach(
      (formula) => {
        if (
    ...
    formelWithoutBlank.forEach((item) => {
      ...
      if (checkNumbers(item)) {
        for (let i: number = 0; i < item.length; i++) {
          if (item.charAt(i) === ".") {
            formelAsObj.push(
              new StrArg(item.charAt(i), false, false, false, false, true)
            );
            finalFormel.usedComma = true;
          } else {
            formelAsObj.push(
              new StrArg(item.charAt(i), false, false, false, true, false)
            );
          }
        }
        notPushed = false;
  ...
  const confirmEdit = () => {
    props.setFormelInformation(
      makeFormelContext(
        currentEditFormel.formelName,
        currentEditFormel.formelString
      )
    );
  ...
  return (
    <StepFrame
      heading={"Bearbeiten der Formeln"}
      hintContent={"Bearbeiten der Formeln!"}
    >
        ...
        >
          ...
          <DialogContent dividers>
            ...
            {formulasToRemove.length !== 0 && (
              <Typography gutterBottom className={classes.wrappedText}>
                Das Löschen der Formel wird folgende Formeln entfernen, da sie
                die Formel nutzen: {formulasToRemove.join(", ")}
                <br /> Durch das Löschen der Formeln können Diagramme und
                Formeln gelöscht werden, die diese nutzen (kaskadierende
                Löschung).
              </Typography>
            )}
          </DialogContent>
