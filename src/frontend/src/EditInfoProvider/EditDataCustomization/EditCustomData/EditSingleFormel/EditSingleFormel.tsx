...
export const EditSingleFormel: React.FC<EditSingleFormelProps> = (props) => {
  ...
  const [input, setInput] = React.useState<string>(
    calculationToString(props.formula.formelAsObjects)
  );
   ...
   */
  const [dataAsObj, setDataAsObj] = React.useState<Array<StrArg>>(
    props.formula.formelAsObjects
  );

  /**
   * dataFlag represents the status of dataButtons. If it ist true the dataButtons will be disabled.
   */
  const [dataFlag, setDataFlag] = React.useState<boolean>(
    props.formula.dataFlag
  );
  /**
   ...
   */
  const [numberFlag, setNumberFlag] = React.useState<boolean>(
    props.formula.numberFlag
  );
  /**
   * rightParenFlag represents the status of rightParenButton. If it ist true the rightParenButton will be disabled.
   */
  const [rightParenFlag, setRightParenFlag] = React.useState<boolean>(
    props.formula.rightParenFlag
  );
  /**
   * leftParenFlag represents the status of leftParenButton. If it ist true the leftParenButton will be disabled.
   */
  const [leftParenFlag, setLeftParenFlag] = React.useState<boolean>(
    props.formula.leftParenFlag
  );

  /**
   * commaFlag represents the status of commaButton. If it is, true the commaButton will be disabled.
   */
  const [commaFlag, setCommaFlag] = React.useState<boolean>(
    props.formula.commaFlag
  );
   ...
   */
  const [leftParenCount, setLeftParenCount] = React.useState<number>(
    props.formula.parenCount
  );
  /**
   * Counts how many right parentheses are included in the formula.
   * Is used to check if the number of right and left parens is even.
   */
  const [rightParenCount, setRightParenCount] = React.useState<number>(
    props.formula.parenCount
  );
  ...
  const usedFormulaAndApiData = React.useRef<Array<string>>(
    props.formula.usedFormulaAndApiData
  );
  ...
  const handleSave = (formel: string) => {
    ...
    if (getListItemsNames(props.listItems).includes(formel)) {
      props.reportError(
        "Fehler: Name wird bereits von einem API-Datum genutzt."
      );
      return;
    }
    ...
    for (let i: number = 0; i < props.arrayProcessingsList.length; i++) {
      if (props.arrayProcessingsList[i].name === formel) {
        props.reportError(
          "Fehler: Name wird bereits von einer Array-Verarbeitung genutzt."
        );
        return;
      }
    }
    ...
    for (let i: number = 0; i < props.stringReplacementList.length; i++) {
      if (props.stringReplacementList[i].name === formel) {
        props.reportError(
          "Fehler: Name wird bereits von einer String-Verarbeitung genutzt."
        );
        return;
      }
    }
  ...
  const handleSuccess = (jsonData: any) => {
    ...
    if (
      oldFormelName.length >= 1 &&
      name !== oldFormelName &&
      searchForNameDuplicate(name)
    ) {
    ...
    if (data.accepted) {
      const arCopy =
        props.infoProvDataSources[props.selectedDataSource].customData.slice();

      for (let i: number = 0; i <= arCopy.length - 1; i++) {
        if (
          arCopy[i].formelName === name ||
          arCopy[i].formelName === oldFormelName
        ) {
          arCopy.splice(i, 1);
        }
      }
  ...
  const handleError = (err: Error) => {
    props.reportError(
      "Fehler: Das Backend antwortet nicht! :  (" + err.message + ")"
    );
  };
   ...
   */
  const sendTestData = useCallFetch(
    "/visuanalytics/testformula",
    {
      method: "POST",
      headers: {
  ...
  const searchForNameDuplicate = (newName: string): boolean => {
    ...
    for (
      let i = 0;
      i <=
      props.infoProvDataSources[props.selectedDataSource].customData.length - 1;
      i++
    ) {
      if (
        props.infoProvDataSources[props.selectedDataSource].customData[i]
          .formelName === newName
      ) {
        foundDuplicate = true;
      }
    ...
    for (
      let i = 0;
      i <=
      props.infoProvDataSources[props.selectedDataSource].selectedData.length -
        1;
      i++
    ) {
      if (
        props.infoProvDataSources[props.selectedDataSource].selectedData[i]
          .key === newName
      ) {
        foundDuplicate = true;
      }
  ...
  return (
    <StepFrame heading="Formeln" hintContent={hintContents.formeln}>
      <Grid
        container
      ...
      >
        <Grid item container xs={12}>
          <EditSingleFormelGUI
            selectedData={
              props.infoProvDataSources[props.selectedDataSource].selectedData
            }
            customData={
              props.infoProvDataSources[props.selectedDataSource].customData
            }
            input={input}
            name={name}
            setName={(name: string) => setName(name)}
            handleOperatorButtons={(operator: string) =>
              handleOperatorButtons(operator)
            }
            handleDataButtons={(operator: string) =>
              handleDataButtons(operator)
            }
