...
export const EditDataSelection: React.FC<EditDataSelectionProps> = (props) => {
  ...
  const [newListItems, setNewListItems] = React.useState<
   ...
   */
  const dataContained = React.useCallback(
    (listItems: Array<ListItemRepresentation>) => {
      ...
      for (
        let index = 0;
        index < props.dataSource.selectedData.length;
        index++
      ) {
        if (!listItemsNames.includes(props.dataSource.selectedData[index].key))
          return false;
      }
      ...
      for (let index = 0; index < props.diagrams.length; index++) {
        ...
        if (
          ...
          for (
            let innerIndex = 0;
            innerIndex < diagram.arrayObjects.length;
            innerIndex++
          ) {
            const arrayObject: ArrayDiagramProperties =
              diagram.arrayObjects[innerIndex];
            //checking is only necessary if the arrayObject is from this api
            if (
              arrayObject.listItem.parentKeyName.split("|")[0] ===
              props.dataSource.apiName
            ) {
              ...
              if (
                !listItemsNames.includes(
                  (arrayObject.listItem.parentKeyName.substring(
                    props.dataSource.apiName.length + 1
                  ).length !== 0
                    ? arrayObject.listItem.parentKeyName.substring(
                        props.dataSource.apiName.length + 1
                      ) + "|"
                    : "") +
                    arrayObject.listItem.keyName +
                    "|0"
                )
              ) {
   ...
   */
  const handleTestDataSuccess = React.useCallback(
    (jsonData: any) => {
      ...
      if (data.status !== 0) {
        ...
        const listItems: Array<ListItemRepresentation> = transformJSON(
          data.api_keys
        );
   ...
   */
  const handleTestDataError = React.useCallback(
    (err: Error) => {
      reportError(
        "Fehler: API-Abfrage über das Backend fehlgeschlagen! (" +
          err.message +
          ")"
      );
      backHandler(1);
    ...
    if (displaySpinner) {
          ...
          >
            ...
            <DialogContent dividers>
              <Typography gutterBottom>
                Einige der ausgewählten oder in Diagrammen verwendeten Daten
                dieser Datenquelle sind nicht in der Antwort der API-Abfrage
                vorhanden.
              </Typography>
              <Typography gutterBottom>
                Dies ist vermutlich darauf zurückzuführen, dass die API ihr
                Datenformat geändert hat oder in ihrer Antwort
                Fehler-Informationen enthält.
              </Typography>
              <Typography gutterBottom>
                Sie können die Datenquelle nicht weiter bearbeiten und die alten
                Einstellungen behalten (sofern sich die Datenquelle tatsächlich
                geändert hat werden API-Abfragen vermutlich Fehler erzeugen).
              </Typography>
              <Typography gutterBottom>
                Alternativ können sie alle Einstellungen und zugehörigen
                Diagramme verwerfen und mit der neuen Antwort weiterarbeiten.
              </Typography>
            </DialogContent>
            <DialogActions>
      ...
      return (
        <DataSelection
          ...
          listItems={
            props.infoProvDataSources[props.selectedDataSource].listItems
          }
  ...
  return <React.Fragment>{selectContent()}</React.Fragment>;
};
