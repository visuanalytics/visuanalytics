...
import {
  createCalculates,
  createReplacements,
  extractKeysFromSelection,
} from "../CreateInfoProvider/helpermethods";
...
export const EditInfoProvider: React.FC<EditInfoProviderProps> = (props) => {
  ...
  const [infoProvId, setInfoProvId] = React.useState(
    props.infoProvId !== undefined ? props.infoProvId : 0
  );
  /**
   ...
   */
  const [infoProvName, setInfoProvName] = React.useState(
    props.infoProvider !== undefined ? props.infoProvider.infoproviderName : ""
  );
  ...
  const [infoProvDataSources, setInfoProvDataSources] = React.useState<
    Array<DataSource>
  >(
    sessionStorage.getItem("infoProvDataSources-" + uniqueId) === null
      ? props.infoProvider!.dataSources
  ...
  const [infoProvDataSourcesKeys, setInfoProvDataSourcesKeys] = React.useState<
    Map<string, DataSourceKey>
  >(
    props.infoProvider !== undefined
      ? props.infoProvider.dataSourcesKeys
      : new Map<string, DataSourceKey>()
  );
  /**
   * The array with diagrams from the Infoprovider that is being edited.
   */
  const [infoProvDiagrams, setInfoProvDiagrams] = React.useState(
    props.infoProvider !== undefined
      ? props.infoProvider.diagrams
      : new Array<Diagram>()
  );
  /**
   ...
   */
  const [formelInformation, setFormelInformation] =
    React.useState<FormelContext>({
      formelName: "",
      parenCount: 0,
  ...
  const [submitInfoProviderDisabled, setSubmitInfoProviderDisabled] =
  ...
  const [refetchDoneList, setRefetchDoneList] = React.useState<Array<boolean>>(
    new Array(
      sessionStorage.getItem("infoProvDataSources-" + uniqueId) === null
        ? props.infoProvider!.dataSources.length
        : JSON.parse(sessionStorage.getItem("infoProvDataSources-" + uniqueId)!)
            .length
    ).fill(false)
  );
  ...
  const checkKeyExistence = React.useCallback(() => {
    ...
    for (let index = 0; index < infoProvDataSources.length; index++) {
      ...
      if (infoProvDataSourcesKeys.get(dataSource.apiName) !== undefined) {
        if (
          !dataSource.noKey &&
          (infoProvDataSourcesKeys.get(dataSource.apiName)!.apiKeyInput1 !==
            "" ||
            infoProvDataSourcesKeys.get(dataSource.apiName)!.apiKeyInput2 !==
              "")
        )
          return true;
      }
  ...
  const authDialogNeeded = () => {
    const data: Array<DataSource> =
      sessionStorage.getItem("infoProvDataSources-" + uniqueId) === null
        ? new Array<DataSource>()
        : JSON.parse(
            sessionStorage.getItem("infoProvDataSources-" + uniqueId)!
          );
  ...
  React.useEffect(() => {
    if (sessionStorage.getItem("firstEntering-" + uniqueId) !== null) {
      //infoProvId
      setInfoProvId(
        Number(sessionStorage.getItem("infoProvId-" + uniqueId) || 0)
      );
      ...
      //newDataSourceMode
      setNewDataSourceMode(
        sessionStorage.getItem("newDataSourceMode-" + uniqueId) === "true"
      );
      //infoProvDataSource doesnt need to be fetched since it works with the initial value
      //infoProvDiagrams
      setInfoProvDiagrams(
        sessionStorage.getItem("infoProvDiagrams-" + uniqueId) === null
          ? new Array<Diagram>()
          : JSON.parse(sessionStorage.getItem("infoProvDiagrams-" + uniqueId)!)
      );
      //selectedDataSource
      setSelectedDataSource(
        Number(sessionStorage.getItem("selectedDataSource-" + uniqueId) || 0)
      );
      //formelInformation
      setFormelInformation(
        sessionStorage.getItem("formelInformation-" + uniqueId) === null
          ? {
              formelName: "",
              parenCount: 0,
              formelAsObjects: new Array<StrArg>(),
      ...
      setHistorySelectionStep(
        Number(sessionStorage.getItem("historySelectionStep-" + uniqueId) || 0)
      );
      //dataCustomizationStep
      setDataCustomizationStep(
        Number(sessionStorage.getItem("dataCustomizationStep-" + uniqueId) || 0)
      );
      //refetchDoneList
      setRefetchDoneList(
        sessionStorage.getItem("refetchDoneList-" + uniqueId) === null
          ? new Array<boolean>()
          : JSON.parse(sessionStorage.getItem("refetchDoneList-" + uniqueId)!)
      );
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "noKey-" + uniqueId,
      newDataSourceMode ? "true" : "false"
    );
  }, [newDataSourceMode]);
  // Store infoProvDataSource in session storage
  React.useEffect(() => {
    sessionStorage.setItem(
      "infoProvDataSources-" + uniqueId,
      JSON.stringify(infoProvDataSources)
    );
  }, [infoProvDataSources]);
  // Store infoProvDiagrams in session storage
  React.useEffect(() => {
    sessionStorage.setItem(
      "infoProvDiagrams-" + uniqueId,
      JSON.stringify(infoProvDiagrams)
    );
  }, [infoProvDiagrams]);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      ...
      selectedDataSource.toString()
    );
  }, [selectedDataSource]);
  // Store infoProvDiagrams in session storage
  React.useEffect(() => {
    sessionStorage.setItem(
      "formelInformation-" + uniqueId,
      JSON.stringify(formelInformation)
    );
  }, [formelInformation]);
  //store historySelectionStep in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "historySelectionStep-" + uniqueId,
      historySelectionStep.toString()
    );
  }, [historySelectionStep]);
  //store dataCustomizationStep in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "dataCustomizationStep-" + uniqueId,
      dataCustomizationStep.toString()
    );
  }, [dataCustomizationStep]);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "refetchDoneList-" + uniqueId,
      JSON.stringify(refetchDoneList)
    );
  ...
  const checkForHistorizedData = () => {
    if (infoProvDataSources[selectedDataSource].historizedData.length <= 0) {
      infoProvDataSources[selectedDataSource].schedule = {
        type: "",
        interval: "",
        time: "",
        weekdays: [],
  ...
  const checkDuplicateNameForEditDataSource = (name: string) => {
    for (let i = 0; i < infoProvDataSources.length; i++) {
      if (infoProvDataSources[i].apiName === name && i !== selectedDataSource)
        return true;
    }
    return false;
  };
  ...
  const setApiKeyInput1 = (key: string) => {
    const key2 = infoProvDataSourcesKeys.get(
      infoProvDataSources[selectedDataSource].apiName
    );
    const keyMap = new Map(infoProvDataSourcesKeys);
    setInfoProvDataSourcesKeys(
      keyMap.set(infoProvDataSources[selectedDataSource].apiName, {
        apiKeyInput1: key,
  ...
  const setApiKeyInput2 = (key: string) => {
    const key1 = infoProvDataSourcesKeys.get(
      infoProvDataSources[selectedDataSource].apiName
    );
    const keyMap = new Map(infoProvDataSourcesKeys);
    setInfoProvDataSourcesKeys(
   ...
   */
  const setStringReplacementList = (
  ...
  const cleanDataSource = (newListItems: Array<ListItemRepresentation>) => {
    ...
    dataSourceCopy.historizedData.forEach((historizedItem) => {
      infoProvDiagrams.forEach((diagram) => {
        ...
        if (
          diagram.sourceType === "Historized" &&
          diagram.historizedObjects !== undefined
        ) {
          for (
            let index = 0;
            index < diagram.historizedObjects.length;
            index++
          ) {
            ...
            if (
              infoProvName + "|" + historizedItem === historized.name &&
              !diagramsToRemove.includes(diagram.name)
            ) {
    ...
    infoProvDiagrams.forEach((diagram) => {
      ...
      if (
        diagram.sourceType === "Array" &&
        diagram.arrayObjects !== undefined
      ) {
    ...
    if (diagramsToRemove.length > 0) {
      setInfoProvDiagrams(
        infoProvDiagrams.filter((diagram) => {
   ...
   */
  const handleSuccessPostInfoProvider = React.useCallback(
    (jsonData: any) => {
   ...
   */
  const handleErrorPostInfoProvider = React.useCallback(
    (err: Error) => {
      setSubmitInfoProviderDisabled(false);
      reportError(
        "Fehler: Senden des Info-Providers an das Backend fehlgeschlagen! (" +
          err.message +
          ")"
      );
    },
        ...
        backendDataSources.push({
          ...
          api: {
            api_info: {
              type: "request",
              api_key_name:
                dataSource.method === "BearerToken"
                  ? infoProvDataSourcesKeys.get(dataSource.apiName)!
                      .apiKeyInput1
                  : infoProvDataSourcesKeys.get(dataSource.apiName)!
                      .apiKeyInput1 +
                    "||" +
          ...
          calculates: createCalculates(
            dataSource.arrayProcessingsList,
            dataSource.apiName
          ),
          replacements: createReplacements(
            dataSource.stringReplacementList,
            dataSource.apiName
          ),
          schedule: {
    ...
    if (diagram.sourceType === "Array") {
      if (diagram.arrayObjects !== undefined) {
        ...
        if (diagram.customLabels) {
          ...
          for (let index = 0; index < diagram.arrayObjects.length; index++) {
            ...
            const plots = {
              ...
              plot: {
                ...
                y:
                  "{_req|" +
                  (item.listItem.parentKeyName === ""
                    ? item.listItem.keyName
                    : item.listItem.parentKeyName +
                      "|" +
                      item.listItem.keyName) +
                  "}",
                ...
                x_ticks: {
                  ...
                  ticks:
                    index === diagram.arrayObjects.length - 1
                      ? diagram.labelArray
                      : [],
                },
              },
          ...
          for (let index = 0; index < diagram.arrayObjects.length; index++) {
            const item = diagram.arrayObjects[index];
            const isArrayWithAttribute =
              (item.listItem.parentKeyName === ""
                ? item.listItem.keyName
                : item.listItem.parentKeyName + "|" + item.listItem.keyName) ===
              diagram.stringAttribute.array;
            const plots = {
              customLabels: false,
              primitive: !Array.isArray(item.listItem.value),
              plot: {
                ...
                color: item.color,
                numericAttribute: item.numericAttribute,
                stringAttribute: isArrayWithAttribute
                  ? diagram.stringAttribute.key
                  : "",
                x_ticks: {
                  //only set the ticks on the last Plot - this is necessary to render it last when layering the Plots in the diagrams
                  ticks: [],
          ...
          const plots = {
            plot: {
              ...
              x_ticks: {
                //only set the ticks on the last Plot - this is necessary to render it last when layering the Plots in the diagrams
                ticks:
                  index === diagram.historizedObjects.length - 1
                    ? diagram.labelArray
                    : [],
              },
            },
    ...
    infoProvDiagrams.forEach((diagram) => {
      if (diagram.sourceType !== "Array") return;
        ...
        diagram.arrayObjects.forEach((array) => {
          ...
          arraysInDiagrams.push(
            array.listItem.parentKeyName + "|" + array.listItem.keyName
          );
        });
      }
    });
  ...
  const postInfoProvider = React.useCallback(() => {
    ...
    infoProvId,
    createBackendDiagrams,
    createDataSources,
    infoProvDiagrams,
    getArraysUsedByDiagrams,
    handleErrorPostInfoProvider,
    handleSuccessPostInfoProvider,
    infoProvName,
  ]);
  ...
  const finishNewDataSource = (
    ...
    setInfoProvDataSourcesKeys(
      mapCopy.set(dataSource.apiName, {
      ...
      case 0:
        return (
          <EditSettingsOverview
            ...
            setInfoProvDataSources={(dataSources: Array<DataSource>) =>
              setInfoProvDataSources(dataSources)
            }
            selectedDataSource={selectedDataSource}
            setSelectedDataSource={(index: number) =>
              setSelectedDataSource(index)
            }
            finishNewDataSource={finishNewDataSource}
            setNewDataSourceMode={setNewDataSourceMode}
            infoProvDiagrams={infoProvDiagrams}
            setInfoProvDiagrams={(diagrams: Array<Diagram>) =>
              setInfoProvDiagrams(diagrams)
            }
            infoProvDataSourcesKeys={infoProvDataSourcesKeys}
            setInfoProvDataSourcesKeys={(keys: Map<string, DataSourceKey>) =>
              setInfoProvDataSourcesKeys(keys)
            }
      ...
      case 1:
        return (
          <EditBasicSettings
            ...
            setDiagrams={(diagrams: Array<Diagram>) =>
              setInfoProvDiagrams(diagrams)
            }
      ...
      case 2:
        return (
          <EditDataSelection
            ...
            setSelectedData={(selectedData: Array<SelectedDataItem>) =>
              setSelectedData(selectedData)
            }
            setHistorizedData={(historizedData: Array<string>) =>
              setHistorizedData(historizedData)
            }
            setCustomData={(customData: Array<FormelObj>) =>
              setCustomData(customData)
            }
            cleanDataSource={(newListItems: Array<ListItemRepresentation>) =>
              cleanDataSource(newListItems)
            }
            infoProvDataSources={infoProvDataSources}
            selectedDataSource={selectedDataSource}
            dataCustomizationStep={dataCustomizationStep}
            setDataCustomizationStep={(step: number) =>
              setDataCustomizationStep(step)
            }
            refetchDoneList={refetchDoneList}
            setRefetchDoneList={(list: Array<boolean>) =>
              setRefetchDoneList(list)
            }
          />
        );
      case 3:
        return (
          <EditDataCustomization
            ...
            selectedDataSource={selectedDataSource}
            infoProvDiagrams={infoProvDiagrams}
            setInfoProvDiagrams={(array: Array<Diagram>) =>
              setInfoProvDiagrams(array)
            }
            ...
            arrayProcessingsList={
              infoProvDataSources[selectedDataSource].arrayProcessingsList
            }
            stringReplacementList={
              infoProvDataSources[selectedDataSource].stringReplacementList
            }
            setHistorizedData={(array: Array<string>) =>
              setHistorizedData(array)
            }
            setSelectedData={(array: Array<SelectedDataItem>) =>
              setSelectedData(array)
            }
            setCustomData={(array: Array<FormelObj>) => setCustomData(array)}
            setArrayProcessingsList={(
              processings: Array<ArrayProcessingData>
            ) => setArrayProcessingsList(processings)}
            setStringReplacementList={(
              replacements: Array<StringReplacementData>
            ) => setStringReplacementList(replacements)}
            finishEditing={finishEditing}
            checkForHistorizedData={checkForHistorizedData}
            formel={formelInformation}
            setFormelInformation={(formelInformation: FormelContext) =>
              setFormelInformation(formelInformation)
            }
          />
        );
      case 4:
        return (
          <HistorySelection
            ...
            selectedData={extractKeysFromSelection(
              infoProvDataSources[selectedDataSource].selectedData
            )}
            customData={infoProvDataSources[selectedDataSource].customData}
            arrayProcessingsList={
              infoProvDataSources[selectedDataSource].arrayProcessingsList
            ...
            historizedData={
              infoProvDataSources[selectedDataSource].historizedData
            }
            ...
            setHistorySelectionStep={(step: number) =>
              setHistorySelectionStep(step)
            }
            diagrams={infoProvDiagrams}
            setDiagrams={(diagrams: Array<Diagram>) =>
              setInfoProvDiagrams(diagrams)
    ...
    <React.Fragment>
      ...
      {authDataDialogOpen && (
        <AuthDataDialog
          ...
          setDataSourcesKeys={(map: Map<string, DataSourceKey>) =>
            setInfoProvDataSourcesKeys(map)
          }
