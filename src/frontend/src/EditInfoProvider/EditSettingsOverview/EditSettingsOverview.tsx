...
export const EditSettingsOverview: React.FC<EditSettingsOverviewProps> = (
  props
) => {
  ...
  const deleteDataSourceHandler = () => {
    ...
    const dataSourceName =
      props.infoProvDataSources[props.selectedDataSource].apiName;
  ...
  const deleteSelectedDataSource = () => {
    ...
    props.setInfoProvDataSources(
      props.infoProvDataSources.filter((dataSource) => {
        return (
          dataSource.apiName !==
          props.infoProvDataSources[props.selectedDataSource].apiName
        );
    ...
    if (diagramsToRemove.current.length > 0) {
      props.setInfoProvDiagrams(
        props.infoProvDiagrams.filter((diagram) => {
        ...
        >
          <Grid item xs={12}>
            <TextField
              ...
              onChange={(event) =>
                props.setInfoProvName(event.target.value.replace(" ", "_"))
              }
            ></TextField>
          </Grid>
          <Grid item xs={12} md={6} className={classes.elementLargeMargin}>
            <Grid item className={classes.blockableButtonDelete}>
              <Button
                disabled={props.infoProvDataSources.length <= 1}
                variant="contained"
                size="large"
              ...
              >
                <List disablePadding={true}>
                  {extractKeysFromSelection(
                    props.infoProvDataSources[props.selectedDataSource]
                      .selectedData
                  ).map((item: string) => renderListItem(item))}
                  {props.infoProvDataSources[
                    props.selectedDataSource
                  ].customData.map((item: FormelObj) =>
                    renderListItem(item.formelName)
                  )}
                </List>
              </Box>
            </Grid>
          ...
          >
            ...
            {props.infoProvDataSources[props.selectedDataSource].historizedData
              .length > 0 && (
              <React.Fragment>
                <Grid item xs={12}>
                  <Box
                  ...
                  >
                    <List disablePadding={true}>
                      {props.infoProvDataSources[
                        props.selectedDataSource
                      ].historizedData.map((item: string) =>
                        renderListItem(item)
                      )}
                    </List>
                  </Box>
                </Grid>
                {props.infoProvDataSources[props.selectedDataSource].schedule
                  .type !== "" && (
                  <Grid item xs={12}>
      ...
      >
        ...
        <DialogContent dividers>
          <Typography gutterBottom className={classes.wrappedText}>
            Die Datenquelle "
            {props.infoProvDataSources[props.selectedDataSource].apiName}" wird
