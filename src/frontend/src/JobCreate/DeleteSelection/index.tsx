...
export const DeleteSelection: React.FC<DeleteSelectionProps> = ({
  deleteSchedule,
  ...
  const handleAddKeepCount = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (
      deleteSchedule.type === "keepCount" &&
      event.target.value.trim() !== ""
    ) {
      selectDeleteScheduleHandler({
        ...deleteSchedule,
        keepCount: Math.floor(Number(event.target.value)),
      });
    }
  };
  ...
  const handleAddFixNames = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (
      deleteSchedule.type === "fixNames" &&
      event.target.value.trim() !== ""
    ) {
      selectDeleteScheduleHandler({
        ...deleteSchedule,
        count: Math.floor(Number(event.target.value)),
      });
    }
  };
      ...
      <div>
        ...
        <div className={classes.MPaddingTB}>
          ...
          <Collapse in={deleteSchedule.type === "onDayHour"}>
            <DayHourInputField
              dayHour={
                deleteSchedule.type === "onDayHour"
                  ? deleteSchedule.removalTime
                  : undefined
              }
          ...
          <Collapse in={deleteSchedule.type === "fixNames"}>
            <div className={classes.MPaddingTB}>
              <TextField
                ...
                onFocus={handleFocus}
                value={
                  deleteSchedule.type === "fixNames" ? deleteSchedule.count : ""
                }
