 ...
 */
export const InfoProviderSelection: React.FC<InfoProviderSelectionProps> = (
  props
) => {
  ...
  const [originalSelectedInfoProvider, setOriginalSelectedInfoProvider] =
    React.useState(props.selectedInfoProvider);
  ...
  React.useEffect(() => {
    if (
      sessionStorage.getItem("firstInfoProvSelectionEntering-" + uniqueId) !==
      null
    ) {
      setOriginalSelectedInfoProvider(
        sessionStorage.getItem("originalSelectedInfoProvider-" + uniqueId) ===
          null
          ? []
          : JSON.parse(
              sessionStorage.getItem(
                "originalSelectedInfoProvider-" + uniqueId
              )!
            )
      );
    } else {
      ...
      sessionStorage.setItem(
        "firstInfoProvSelectionEntering-" + uniqueId,
        "false"
      );
    }
  }, []);
  React.useEffect(() => {
    sessionStorage.setItem(
      "originalSelectedInfoProvider-" + uniqueId,
      JSON.stringify(originalSelectedInfoProvider)
    );
  ...
  const handleSuccessFetchById = (jsonData: any) => {
    ...
    infoProviderToFetch.current = infoProviderToFetch.current.filter(
      (infoProvider) => {
  ...
  const checkProceedDialog = () => {
    ...
    if (props.selectedInfoProvider.length < originalSelectedInfoProvider.length)
      return true;
  ...
  const checkBoxHandler = (infoProvider: InfoProviderData) => {
    ...
    if (checkIdIncluded(infoProvider.infoprovider_id)) {
      ...
      props.setSelectedInfoProvider(
        props.selectedInfoProvider.filter((selectedInfoProvider) => {
          return (
            selectedInfoProvider.infoprovider_id !==
            infoProvider.infoprovider_id
          );
        })
      );
      // Remove the infoprovider also from the array with the IDs of all selected infoproviders
      props.setInfoproviderIDs(
        props.infoproviderIDs.filter((selectedInfoProviderID) => {
      ...
      <Grid container justify="space-around">
        <Grid item xs={12}>
          <Typography variant="body1">
            Bitte wählen sie die Infoprovider, deren Datenwerte in der
            Videoerstellung für Text-to-Speech zur Verfügung stehen sollen:
          </Typography>
        </Grid>
        <Grid item xs={10}>
          <Box
          ...
          >
            <List disablePadding={true}>
              {props.infoProviderList.map((infoProvider) =>
                renderListItem(infoProvider)
              )}
            </List>
          </Box>
        </Grid>
        <Grid item xs={12} className={classes.fixedWarningContainer}>
          {props.selectedInfoProvider.length > 5 && (
            <Alert severity="warning">
              <strong>Warnung:</strong> Es müssen die Informationen aller
              ausgewählten Infoprovider gleichzeitig geladen werden.
              <br />
              Bitte berücksichtigen sie, dass bei einer größeren Menge und sehr
              großen Infoprovidern Performance-Einbußen auftreten können.
            </Alert>
          )}
        </Grid>
        <Grid
      ...
      >
        ...
        <DialogContent dividers>
          <Typography gutterBottom>
            Durch das Abwählen von Infoprovidern wird es notwendig, die bisher
            vorgenommenen Szeneneinstellungen zurücksetzen.
          </Typography>
          <Typography gutterBottom>Wirklich fortfahren?</Typography>
        </DialogContent>
        <DialogActions>
