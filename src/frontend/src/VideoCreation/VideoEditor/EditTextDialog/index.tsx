...
export const EditTextDialog: React.FC<EditTextDialogProps> = (props) => {
  ...
  const [showInfoproviderData, setShowInfoproviderData] = React.useState<
    Array<boolean>
  >(new Array(props.minimalInfoproviders.length).fill(false));
  ...
  const [lastEditedTTSTextIndex, setLastEditedTTSTextIndex] =
    React.useState<number>(0);

  // State, that defines, if the dialog for setting intervals for historized data should be shown or not
  const [showAddHistorizedDialog, setShowHistorizedDialog] =
  ...
  const insertSelectedOrCustomData = (item: string, noReq: boolean) => {
    ...
    arrCopy[lastEditedTTSTextIndex].text =
      arrCopy[lastEditedTTSTextIndex].text +
      (!noReq ? "{_req|" : "{") +
      item +
      "} ";
  ...
  const insertHistorizedData = () => {
    if (selectedInterval !== undefined) {
      ...
      arrCopy[lastEditedTTSTextIndex].text =
        arrCopy[lastEditedTTSTextIndex].text +
        "{_req|" +
        selectedHistorizedElement.replaceAll("|", "_") +
        "_HISTORY|" +
  ...
  const changeElement = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const arrCopy = audioElements.slice();
    if (arrCopy[editedElement].type === "text")
      arrCopy[editedElement].text = event.target.value;
    else if (arrCopy[editedElement].type === "pause")
      arrCopy[editedElement].duration = Number(event.target.value);
    setAudioElements(arrCopy);
  };
  ...
  const isInvalidCombination = () => {
    for (let i = 0; i < audioElements.length; i++) {
      if (
        audioElements[i].type === "text" &&
        (audioElements[i].text === "" ||
          audioElements[i].text!.replace(/\s/g, "").length === 0)
      )
        return true;
      else if (
        audioElements[i].type === "pause" &&
        (audioElements[i].duration === undefined ||
          audioElements[i].duration! < 0)
      )
        return true;
    }
    return false;
      ...
      return (
        ...
        >
          <Grid item xs={12} md={10}>
            <TextField
              fullWidth
              error={audioElement.text === ""}
              id={id.toString()}
              label="TTS-Text"
              multiline
              rowsMax={3}
              value={audioElement.text}
              onChange={changeElement}
              onClick={() => handleClickOnTTSTextField(id)}
            />
          </Grid>
          <Grid item xs={12} md={2}>
            <IconButton
              aria-label="Abschnitt löschen"
              disabled={audioElements.length <= 2}
              onClick={() => deleteText(id)}
              className={classes.redDeleteIcon}
            >
              <DeleteIcon />
            </IconButton>
          </Grid>
      ...
      return (
        <Grid key={id} item xs={10} className={classes.elementLargeMargin}>
          <TextField
            fullWidth
            error={
              audioElement.duration === undefined || audioElement.duration < 0
            }
            id={id.toString()}
            label="Pause in ms"
            type="number"
            defaultValue={audioElement.duration}
            onChange={changeElement}
          />
        </Grid>
      );
    ...
    return (
      <ListItem key={item}>
        <Button
          variant="contained"
          size="large"
          onClick={() => handleClickOnHistorizedData(item, schedule)}
        >
          <span className={classes.overflowButtonText}>
            {infoProvName + "|" + item}
          </span>
        </Button>
      </ListItem>
    ...
    return (
      <ListItem key={item}>
        <Button
          ...
          size="large"
          onClick={() =>
            insertSelectedOrCustomData(
              withoutDataSourceName !== undefined
                ? withoutDataSourceName
                : item,
              noReq
            )
          }
  ...
  const renderDataSource = (
    ...
    return (
      <Grid item container xs={12} key={dataSource.apiName}>
        ...
        <Grid item xs={12}>
          <List
            key={dataSource.apiName + "-SelectedData"}
            disablePadding={true}
          >
            {extractKeysFromSelection(dataSource.selectedData).map(
              (item: string) =>
                renderData(
                  dataSource.apiName + "|" + item,
                  infoProviderName,
                  false
                )
            )}
        ...
        <Grid item xs={12}>
          <List
            ...
            disablePadding={true}
          >
            {dataSource.arrayProcessingList.map((item: ArrayProcessingData) =>
              renderData(
                dataSource.apiName + "|" + item.name,
                infoProviderName,
                true,
                item.name
              )
        ...
        <Grid item xs={12}>
          <List
            ...
            disablePadding={true}
          >
            {dataSource.stringReplacementList.map(
              (item: StringReplacementData) =>
                renderData(
                  dataSource.apiName + "|" + item.name,
                  infoProviderName,
                  true,
        ...
        <Grid item xs={12} className={classes.elementLargeMargin}>
          <List key={dataSource.apiName + "-CustomData"} disablePadding={true}>
            {dataSource.customData.map((item: FormelObj) =>
              renderData(
                dataSource.apiName + "|" + item.formelName,
                infoProviderName,
                true,
                item.formelName
              )
        ...
        <Grid item xs={12}>
          <List
            key={dataSource.apiName + "-HistorizedData"}
            disablePadding={true}
          >
            {dataSource.historizedData.map((item: string) =>
              renderHistorizedData(
                dataSource.apiName + "|" + item,
                infoProviderName,
                dataSource.schedule
              )
   ...
   */
  const renderInfoproviderData = (
    dataSources: Array<MinimalDataSource>,
    infoproviderName: string,
    infoproviderIndex: number
  ) => {
    return (
      <Grid key={infoproviderName} item container xs={12}>
        <Grid item container xs={12}>
          ...
          <Grid item xs={12} md={2}>
            {!showInfoproviderData[infoproviderIndex] && (
              <IconButton
                aria-label="Infoprovider-Daten ausklappen"
                onClick={() => toggleInfoproviderData(infoproviderIndex)}
              >
                <ExpandMore />
              </IconButton>
            )}
            {showInfoproviderData[infoproviderIndex] && (
              <IconButton
                aria-label="Infoprovider-Daten einklappen"
                onClick={() => toggleInfoproviderData(infoproviderIndex)}
              >
                <ExpandLess />
              </IconButton>
        ...
        <Grid item xs={12}>
          <Collapse in={showInfoproviderData[infoproviderIndex]}>
            {dataSources.map((dataSource: MinimalDataSource) =>
              renderDataSource(dataSource, infoproviderName)
            )}
          </Collapse>
        </Grid>
        <Grid item xs={12}>
  ...
  return (
    <React.Fragment>
      <Dialog
        aria-labelledby="EditTextDialog-Title"
        maxWidth="md"
        open={props.openEditTextDialog}
      >
        <DialogTitle id="EditTextDialog-Title">
          Text für {props.sceneName} bearbeiten
        </DialogTitle>
        <DialogContent dividers>
          <Grid container>
            <Grid item xs={12}>
              Fügen Sie hier Texte und Pausen zur gewählten Szene{" "}
              <strong>{props.sceneName}</strong> hinzu.
            </Grid>
            <Grid
              container
              justify="space-between"
            ...
            >
              <Grid item xs={12} md={5}>
                <Box
                  ...
                  border={4}
                  borderRadius={5}
                  className={classes.editDialogListFrame}
                >
                  <Grid item xs={12}>
                    <Typography variant="h5">Elemente</Typography>
                  </Grid>
                  {audioElements.map(
                    (audioElement: AudioElement, index: number) =>
                      renderEditElement(index, audioElement)
                  )}
                  <Grid item xs={12}>
                    <Divider className={classes.greyDivider} />
                  </Grid>
                  <Grid item xs={12} className={classes.elementLargeMargin}>
                    <Typography variant="h6">Neuer Text-Abschnitt</Typography>
                  </Grid>
                  <Grid item xs={10}>
                    <TextField
                      fullWidth
                      error={newPause === undefined || newPause < 0}
                      label="Pause vor nächstem Text in ms"
                      type="number"
                      value={newPause}
                      onChange={(event) =>
                        setNewPause(
                  ...
                  <Grid
                    item
                    xs={12}
                    className={classes.blockableButtonPrimaryMarginTop}
                  >
                    <Button
                      variant="contained"
                      color="primary"
                      disabled={
                        newPause === undefined ||
                        newPause < 0 ||
                        audioElements[audioElements.length - 1].text === "" ||
                        audioElements[audioElements.length - 1].text!.replace(
                          /\s/g,
                          ""
                        ).length === 0
                      }
              ...
              <Grid item container xs={12} md={6}>
                ...
                >
                  <Grid item xs={12}>
                    <Typography variant="h5">API-Daten</Typography>
                  </Grid>
                  <Grid item container xs={12}>
                    {props.minimalInfoproviders.map(
                      (
                        minimalInfoprovider: MinimalInfoProvider,
                        infoproviderIndex: number
                      ) =>
                        renderInfoproviderData(
                          minimalInfoprovider.dataSources,
                          minimalInfoprovider.infoproviderName,
                          infoproviderIndex
                        )
                    )}
                  </Grid>
          ...
          <Grid container justify="space-between">
            <Grid item>
              <Button
                variant="contained"
                color="primary"
                onClick={() => props.setOpenEditTextDialog(false)}
              >
                Abbrechen
              </Button>
            </Grid>
            <Grid item className={classes.blockableButtonSecondary}>
              <Button
                variant="contained"
                color="secondary"
                disabled={isInvalidCombination()}
                onClick={saveNewAudio}
              >
                Speichern
              </Button>
            </Grid>
          ...
          <Grid container>
            <Grid item xs={12}>
              <Typography variant="body1">
                Legen Sie bitte das Intervall fest, welches für die TTS
                verwendet werden soll. Die Zahl 0 meint dabei das aktuellste
                Intervall, die Zahl 1 das Vorletzte, usw.
              </Typography>
            </Grid>
            <Grid item xs={12} className={classes.elementLargeMargin}>
              <Typography variant="h6">
                Informationen zu Historisierungszeitpunkten des gewählten
                Elements:
              </Typography>
            </Grid>
            <Grid item xs={12} className={classes.elementLargeMargin}>
            ...
            <Grid item xs={12} className={classes.elementLargeMargin}>
              <TextField
                className={classes.ttsIntervalInput}
                error={selectedInterval === undefined || selectedInterval < 0}
          ...
          <Grid item container justify="space-between">
            <Grid item>
              <Button
                variant="contained"
                color="primary"
                onClick={() => cancelHistorizedDataAdding()}
              >
                Abbrechen
              </Button>
            </Grid>
            <Grid item className={classes.blockableButtonSecondary}>
              <Button
                ...
                color="secondary"
                disabled={
                  selectedInterval === undefined || selectedInterval < 0
                }
                onClick={() => insertHistorizedData()}
              >
                Speichern
              </Button>
            </Grid>
