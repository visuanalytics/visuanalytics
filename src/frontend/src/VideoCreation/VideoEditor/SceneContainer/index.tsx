...
export const SceneContainer: React.FC<SceneContainerProps> = (props) => {
  ...
  const removeScene = (index: number) => {
    ...
    props.setSceneList(
      props.sceneList.slice(0, index).concat(props.sceneList.slice(index + 1))
    );
  };
  ...
  const moveScene = (sourceIndex: number, direction: Direction) => {
    ...
    if (
      (sourceIndex < 1 && direction === Direction.Left) ||
      (sourceIndex >= props.sceneList.length - 1 &&
  ...
  const renderSceneEntry = (sceneEntry: SceneCardData, index: number) => {
    ...
    return (
      <ListItem key={sceneEntry.entryId}>
        <Collapse
          in={sceneEntry.visible}
          timeout={{ appear: 500, enter: 800, exit: 0 }}
        >
          <React.Fragment>
            <SceneCard
              ...
              exceedDisplayDuration={
                props.sceneList[index].exceedDisplayDuration
              }
              setExceedDisplayDuration={(newDisplayDuration: number) =>
                setDisplayDuration(index, newDisplayDuration)
              }
              spokenText={props.sceneList[index].spokenText}
              setSpokenText={(newSpokenText: Array<AudioElement>) =>
                setSpokenText(index, newSpokenText)
              }
  ...
  return (
    <Box
      ...
      border={4}
      className={classes.sceneContainerBox}
    >
      <List style={{ display: "flex", flexDirection: "row", padding: 0 }}>
        {props.sceneList.map((sceneEntry, index) =>
          renderSceneEntry(sceneEntry, index)
        )}
      </List>
    </Box>
  );
