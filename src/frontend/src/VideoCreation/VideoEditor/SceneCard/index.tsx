...
export const SceneCard: React.FC<SceneCardProps> = (props) => {
  ...
  const [localExceedDisplayDuration, setLocalExceedDisplayDuration] =
    React.useState(props.exceedDisplayDuration);
  //timeout for setting the props value of exceedDisplayDuration shortly after the user has stopped input
  const [timeoutExceedDisplayDuration, setTimeoutExceedDisplayDuration] =
    React.useState(0);
  ...
  const getSpokenTextBeginning = () => {
    if (props.spokenText.length === 0 || props.spokenText[0].type !== "text")
      return "";
    if (props.spokenText[0].text !== undefined)
      return props.spokenText[0].text.length <= 20
        ? props.spokenText[0].text
        : props.spokenText[0].text.slice(0, 20) + "...";
  };
   ...
   */
  const handleExceedDurationSliderChange = (
    event: object,
   ...
   */
  const handleExceedDurationInputChange = (
  ...
  const changePropsExceedDisplayDuration = () => {
    ...
    setTimeoutExceedDisplayDuration(
      window.setTimeout(() => {
      ...
      <Card variant="outlined" color="primary" className={classes.sceneCard}>
          ...
          <Grid item container>
            ...
            <Grid item container xs={12} className={classes.elementLargeMargin}>
              <Grid item xs={12}>
                <Typography
                  variant="body1"
                  id={props.sceneName + "-durationExceed-input"}
                >
              ...
              <Grid item xs={12}>
                <Slider
                  ...
                  getAriaValueText={() =>
                    props.exceedDisplayDuration + " Sekunden"
                  }
              ...
              <Grid item xs={3}>
                <Input
                  ...
                  inputProps={{
                    ...
                    type: "number",
                    "aria-labelledby":
                      props.sceneName + "-durationExceed-input",
                  }}
                />
              </Grid>
              <Grid item xs={9}>
                <Typography variant="body1">Sekunden</Typography>
              </Grid>
            </Grid>
        ...
        <CardActions>
          <Grid container justify="space-between">
            ...
            <Grid item>
              <IconButton
                aria-label="Szene nach links bewegen"
            ...
            <Grid item>
              <IconButton
                aria-label="Szene nach rechts bewegen"
