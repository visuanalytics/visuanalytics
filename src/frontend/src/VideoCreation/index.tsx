...
import { Schedule, uniqueId } from "../CreateInfoProvider/types";
...
export const VideoCreation: React.FC<VideoCreationProps> = (props) => {
  ...
  const [selectedInfoProvider, setSelectedInfoProvider] = React.useState<
    Array<InfoProviderData>
  >(props.video ? props.video.selectedInfoprovider : []);
  ...
  const [minimalInfoProvObjects, setMinimalInfoProvObjects] = React.useState<
    Array<MinimalInfoProvider>
  >([]);
  ...
  const [sceneList, setSceneList] = React.useState<Array<SceneCardData>>(
    props.video ? props.video.sceneList : []
  );
  ...
  const [editModeFlag, setEditModeFlag] = React.useState(
   ...
   */
  const handleErrorFetchInfoProvider = React.useCallback(
    (err: Error) => {
  ...
  React.useEffect(() => {
    ...
    if (
      sessionStorage.getItem("firstVideoCreationEntering-" + uniqueId) !== null
    ) {
      //editModeFlag
      setEditModeFlag(
        sessionStorage.getItem("editModeFlag-" + uniqueId) === "true"
      );
      //propsVideo
      setPropsVideo(
        sessionStorage.getItem("propsVideo-" + uniqueId) === null
          ? undefined
          : JSON.parse(sessionStorage.getItem("propsVideo-" + uniqueId)!)
      );
      //propsVideoId
      setPropsVideoId(
        sessionStorage.getItem("propsVideoId-" + uniqueId) === null
          ? undefined
          : Number(sessionStorage.getItem("propsVideoId-" + uniqueId)!)
      );
      //selectedInfoProvider
      setSelectedInfoProvider(
        sessionStorage.getItem("selectedInfoProvider-" + uniqueId) === null
          ? new Array<InfoProviderData>()
          : JSON.parse(
              sessionStorage.getItem("selectedInfoProvider-" + uniqueId)!
            )
      );
      //videoJobName
      ...
      //sceneList
      setSceneList(
        sessionStorage.getItem("sceneList-" + uniqueId) === null
          ? []
    ...
    if (sessionStorage)
      //videoCreationStep
      setVideoCreationStep(
        Number(sessionStorage.getItem("videoCreationStep-" + uniqueId) || 0)
      );
    //infoProviderList
    setInfoProviderList(
    ...
    setMinimalInfoProvObjects(
      sessionStorage.getItem("minimalInfoProvObjects-" + uniqueId) === null
        ? new Array<MinimalInfoProvider>()
        : JSON.parse(
            sessionStorage.getItem("minimalInfoProvObjects-" + uniqueId)!
          )
    );
    //availableScenes
    setAvailableScenes(
      sessionStorage.getItem("availableScenes-" + uniqueId) === null
        ? []
        : JSON.parse(sessionStorage.getItem("availableScenes-" + uniqueId)!)
    );
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "videoCreationStep-" + uniqueId,
      videoCreationStep.toString()
    );
  }, [videoCreationStep]);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "selectedInfoProvider-" + uniqueId,
      JSON.stringify(selectedInfoProvider)
    );
  }, [selectedInfoProvider]);
  //store minimalInfoProvObjects in sessionStorage by using JSON.stringify on it
  React.useEffect(() => {
    sessionStorage.setItem(
      "minimalInfoProvObjects-" + uniqueId,
      JSON.stringify(minimalInfoProvObjects)
    );
  }, [minimalInfoProvObjects]);
  //store availableScenes in sessionStorage by using JSON.stringify on it
  React.useEffect(() => {
    sessionStorage.setItem(
      "availableScenes-" + uniqueId,
  ...
  React.useEffect(() => {
    if (propsVideoId !== undefined)
      sessionStorage.setItem(
        "propsVideoId-" + uniqueId,
        propsVideoId.toString()
      );
  }, [propsVideoId]);
   ...
   */
  const fetchAllScenes = useCallFetch(
    "/visuanalytics/scene/all",
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json\n",
  ...
  const sendVideoToBackend = () => {
    let url =
      propsVideo && propsVideoId
        ? "visuanalytics/videojob/" + propsVideoId
        : "visuanalytics/videojob";
    //if this variable is set, add it to the url
    if (process.env.REACT_APP_VA_SERVER_URL)
      url = process.env.REACT_APP_VA_SERVER_URL + url;
    //setup a timer to stop the request after 5 seconds
      ...
      case 0: {
        return (
          <InfoProviderSelection
            ...
            selectedInfoProvider={selectedInfoProvider}
            setSelectedInfoProvider={(selection: Array<InfoProviderData>) =>
              setSelectedInfoProvider(selection)
            }
            setMinimalInfoProvObjects={(objects: Array<MinimalInfoProvider>) =>
              setMinimalInfoProvObjects(objects)
            }
      ...
      case 1: {
        return (
          <VideoEditor
            ...
            sceneList={sceneList}
            setSceneList={(sceneList: Array<SceneCardData>) =>
