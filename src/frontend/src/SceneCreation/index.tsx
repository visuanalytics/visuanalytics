...
export const SceneCreation: React.FC<SceneCreationProps> = (props) => {
  ...
  const [sceneEditorStep, setSceneEditorStep] = React.useState(
    props.sceneFromBackend !== undefined ? 1 : 0
  );
  //the list of all infoproviders fetched from the backend
  const [infoProviderList, setInfoProviderList] = React.useState<
    Array<InfoProviderData>
  >([]);
  //object of the infoprovider to be used in the scene creation, selected in first step
  const [infoProvider, setInfoProvider] = React.useState<FrontendInfoProvider>(
    {} as FrontendInfoProvider
  );
  ...
  const [historizedDataList, setHistorizedDataList] = React.useState<
    Array<HistorizedDataInfo>
  >([]);
  const [arrayProcessingList, setArrayProcessingList] = React.useState<
    Array<string>
  >([]);
  const [stringReplacementList, setStringReplacementList] = React.useState<
    Array<string>
  >([]);
  ...
  const [sceneFromBackend, setSceneFromBackend] = React.useState(
    props.sceneFromBackend
  );
  ...
  const [fetchImageDialogOpen, setFetchImageDialogOpen] = React.useState(
    props.sceneFromBackend !== undefined
  );

  // contains the names of the steps to be displayed in the stepper
  const steps = ["Infoprovider-Auswahl", "Szenen-Erstellung"];
  ...
  const fetchNextBackgroundImage = () => {
    ...
    if (allBackgroundImageList.current.length === 0) {
      ...
      allBackgroundImageList.current = allBackgroundImageList.current.filter(
        (image) => {
   ...
   */
  const fetchBackgroundImageById = (
    id: number,
    image_url: string,
   ...
   * as additional argument (storing in state is no alternative because there wont be re-render).
   */
  const fetchImageById = React.useCallback(
    (
      id: number,
      image_url: string,
      successHandler: (jsonData: any, id: number, url: string) => void,
   ...
   */
  const diagramPreviewSuccessHandler = (
    jsonData: any,
    diagram: DiagramInfo
  ) => {
  ...
  const fetchDiagramPreview = (diagram: DiagramInfo) => {
    ...
    let url =
      "/visuanalytics/infoprovider/" +
      (sceneFromBackend !== undefined
        ? sceneFromBackend.used_infoproviders[0]
        : selectedId) +
      "/" +
      diagram.name;
    //if this variable is set, add it to the url
  ...
  React.useEffect(() => {
    //step
    setSceneEditorStep(
      Number(sessionStorage.getItem("sceneEditorStep-" + uniqueId) || 0)
    );
    //infoProviderList
    setInfoProviderList(
      sessionStorage.getItem("infoProviderList-" + uniqueId) === null
        ? new Array<InfoProviderData>()
        : JSON.parse(sessionStorage.getItem("infoProviderList-" + uniqueId)!)
    );
    //infoProvider
    setInfoProvider(
      sessionStorage.getItem("infoProvider-" + uniqueId) === null
        ? new Array<DataSource>()
        : JSON.parse(sessionStorage.getItem("infoProvider-" + uniqueId)!)
    );
    //selectedDataList
    setSelectedDataList(
      sessionStorage.getItem("selectedDataList-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("selectedDataList-" + uniqueId)!)
    );
    //selectedDataList
    setCustomDataList(
      sessionStorage.getItem("customDataList-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("customDataList-" + uniqueId)!)
    );
    //selectedDataList
    setHistorizedDataList(
      sessionStorage.getItem("historizedDataList-" + uniqueId) === null
        ? new Array<HistorizedDataInfo>()
        : JSON.parse(sessionStorage.getItem("historizedDataList-" + uniqueId)!)
    );
    //arrayProcessingsList
    setArrayProcessingList(
      sessionStorage.getItem("arrayProcessingList-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("arrayProcessingList-" + uniqueId)!)
    );
    //stringReplacementList
    setStringReplacementList(
      sessionStorage.getItem("stringReplacementList-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(
            sessionStorage.getItem("stringReplacementList-" + uniqueId)!
          )
    );
    //diagramList
    setDiagramList(
      sessionStorage.getItem("diagramList-" + uniqueId) === null
        ? new Array<DiagramInfo>()
        : JSON.parse(sessionStorage.getItem("diagramList-" + uniqueId)!)
    );
    //selectedId
    setSelectedId(
      Number(sessionStorage.getItem("selectedId-" + uniqueId) || 0)
    );

    //dont set the data for editing when fetching first
    if (
      sessionStorage.getItem("firstSceneCreationEntering-" + uniqueId) !== null
    ) {
      //fetchImageDialogOpen
      setFetchImageDialogOpen(
        sessionStorage.getItem("fetchImageDialogOpen-" + uniqueId) === "true"
      );
      //editId
      setEditId(
        Number(sessionStorage.getItem("editId-" + uniqueId)) || undefined
      );
      //sceneFromBackend
      setSceneFromBackend(
        sessionStorage.getItem("sceneFromBackend-" + uniqueId) === null
          ? undefined
          : JSON.parse(sessionStorage.getItem("sceneFromBackend-" + uniqueId)!)
      );
    ...
    if (
      Number(sessionStorage.getItem("sceneEditorStep-" + uniqueId) || 0) === 1
    ) {
      // set the list of diagrams to fetch by getting the data from sessionStorage:
      diagramsToFetch.current =
        sessionStorage.getItem("diagramList-" + uniqueId) === null
          ? new Array<DiagramInfo>()
          : JSON.parse(sessionStorage.getItem("diagramList-" + uniqueId)!);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "sceneEditorStep-" + uniqueId,
      sceneEditorStep.toString()
    );
  }, [sceneEditorStep]);
  //store infoProviderList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "infoProviderList-" + uniqueId,
      JSON.stringify(infoProviderList)
    );
  }, [infoProviderList]);
  //store infoProvider in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "infoProvider-" + uniqueId,
      JSON.stringify(infoProvider)
    );
  }, [infoProvider]);
  //store selectedDataList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "selectedDataList-" + uniqueId,
      JSON.stringify(selectedDataList)
    );
  }, [selectedDataList]);
  //store customDataList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "customDataList-" + uniqueId,
      JSON.stringify(customDataList)
    );
  }, [customDataList]);
  //store historizedDataList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "historizedDataList-" + uniqueId,
      JSON.stringify(historizedDataList)
    );
  }, [historizedDataList]);
  //store arrayProcessingList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "arrayProcessingList-" + uniqueId,
      JSON.stringify(arrayProcessingList)
    );
  }, [arrayProcessingList]);
  //store stringReplacementList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "stringReplacementList-" + uniqueId,
      JSON.stringify(stringReplacementList)
    );
  }, [stringReplacementList]);
  //store diagramList in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "diagramList-" + uniqueId,
      JSON.stringify(diagramList)
    );
  }, [diagramList]);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "fetchImageDialogOpen-" + uniqueId,
      fetchImageDialogOpen ? "true" : "false"
    );
  }, [fetchImageDialogOpen]);
  ...
  React.useEffect(() => {
    ...
    if (sceneFromBackend !== undefined)
      sessionStorage.setItem(
        "sceneFromBackend-" + uniqueId,
        JSON.stringify(sceneFromBackend)
      );
  }, [sceneFromBackend]);
  ...
  React.useEffect(() => {
    ...
    if (sceneFromBackendMutable.current !== undefined) {
      ...
      const sceneItemsCopy =
        sceneFromBackendMutable.current.scene_items.slice();
      ...
      if (sceneFromBackendMutable.current.infoProvider !== undefined) {
        diagramsToFetch.current = getDiagramFetchList(
          sceneFromBackendMutable.current.infoProvider.diagrams
        );
      }
      ...
      case 0:
        return props.sceneFromBackend !== undefined ? (
          ...
          <InfoProviderSelection
            ...
            setInfoProvider={(infoProvider: FrontendInfoProvider) =>
              setInfoProvider(infoProvider)
            }
            setSelectedDataList={(list: Array<string>) =>
              setSelectedDataList(list)
            }
            setCustomDataList={(list: Array<string>) => setCustomDataList(list)}
            setHistorizedDataList={(list: Array<HistorizedDataInfo>) =>
              setHistorizedDataList(list)
            }
            setArrayProcessingList={(list: Array<string>) =>
              setArrayProcessingList(list)
            }
            setStringReplacementList={(list: Array<string>) =>
              setStringReplacementList(list)
            }
            ...
            setStep0ContinueDisabled={(disabled: boolean) =>
              setStep0ContinueDisabled(disabled)
            }
            selectedId={selectedId}
            setSelectedId={(id: number) => setSelectedId(id)}
            diagramsToFetch={diagramsToFetch}
            displayLoadMessage={displayLoadMessage}
            setDisplayLoadMessage={(display: boolean) =>
              setDisplayLoadMessage(display)
            }
          />
        );
      case 1:
        ...
        if (displaySpinner) {
          return (
            ...
            >
              {!fetchImageDialogOpen && (
                <React.Fragment>
                  <Typography>
                    Bilder, Hintergrundbilder und Diagramme werden erneut
                    geladen...
                  </Typography>
                  <CircularProgress />
                </React.Fragment>
          ...
          return (
            <SceneEditor
              ...
              imageList={imageList}
              setImageList={(images: Array<ImageFrontendData>) =>
                setImageList(images)
              }
              backgroundImageList={backgroundImageList}
              setBackgroundImageList={(backgrounds: Array<ImageFrontendData>) =>
                setBackgroundImageList(backgrounds)
              }
              editMode={false}
  ...
  return (
    <React.Fragment>
      ...
      {!fetchImageDialogOpen && selectContent(sceneEditorStep)}
      <CenterNotification
      ...
      {fetchImageDialogOpen && (
        ...
        >
          ...
          >
            <DialogTitle id="backDialog-title">Bilder erneut laden</DialogTitle>
            <DialogContent dividers>
              <Typography gutterBottom>
                Durch das Neuladen der Seite müssen alle Bilder,
                Hintergrundbilder und Diagramme erneut geladen werden.
              </Typography>
            </DialogContent>
            <DialogActions>
