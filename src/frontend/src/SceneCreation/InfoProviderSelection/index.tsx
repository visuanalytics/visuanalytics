...
export const InfoProviderSelection: React.FC<InfoProviderSelectionProps> = (
  ...
  const fetchInfoProviderById = useCallFetch(
    "visuanalytics/infoprovider/" + props.selectedId,
    {
      method: "GET",
      headers: {
    ...
    handleFetchInfoProviderSuccess,
    handleFetchInfoProviderError
  );
  ...
  const renderListItem = (infoProvider: InfoProviderData) => {
    return (
      <FormControlLabel
        ...
        value={infoProvider.infoprovider_id}
        control={<Radio />}
        label={infoProvider.infoprovider_name}
        key={infoProvider.infoprovider_id}
      />
    );
  };
      ...
      <Grid container>
        <Grid item xs={12}>
          <Typography variant="body1">
            Wählen sie den Infoprovider aus, dessen Daten in der
            Szenenerstellung zur Verfügung stehen sollen.
          </Typography>
        </Grid>
        ...
        <Grid item xs={12} className={classes.fixedWarningContainer}>
          {props.displayLoadMessage && (
            <Alert severity="info">
              ...
              <br /> Dies kann einen Moment dauern.
            </Alert>
          )}
        </Grid>
        <Grid
        ...
        >
          ...
          <Grid item className={classes.blockableButtonPrimary}>
            <Button
              disabled={props.step0ContinueDisabled || props.selectedId === 0}
              variant="contained"
              size="large"
              color={"primary"}
              onClick={() => {
