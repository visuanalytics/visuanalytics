...
interface DiagramListProps {
    ...
    handleDiagramClick: (src : string, id: number, path: string, index: number) => void;
    ...
    const renderDiagramEntry = (diagram: DiagramInfo, index: number) => {
        return (
            <Grid key={diagram.name} item container xs={6} justify="space-around" className={index === 0 ? classes.firstImage : index === 1 ? classes.secondImage : index % 2 === 0 ? classes.leftImage : classes.rightImage}>
                <Grid item xs={10}>
                    <img src={diagram.url} className={classes.imageInList} alt={"Image Nr." +  index} onClick={() => props.handleDiagramClick(diagram.url, 0, props.infoProviderName + "_" + diagram.name, index)}/>
                </Grid>
                <Grid item xs={10}>
