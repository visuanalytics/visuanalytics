        ...
        >
          <Grid item xs={10}>
            <img
              ...
              alt={"Image Nr." + index}
              onClick={() =>
                props.handleImageClick(image, id, path, index, false, "")
              }
            />
          </Grid>
        </Grid>
        ...
        >
          <Grid item xs={10}>
            <img
              ...
              className={classes.imageInList}
              alt={"Image Nr." + index}
              onClick={() => props.handleBackgroundImageClick(image, index)}
            />
          </Grid>
        </Grid>
      );
  ...
  return (
    <React.Fragment>
      <Grid item container xs={12}>
        ...
        <Grid item xs={2}>
          {!showBackgroundImages && (
            <IconButton
              aria-label="Hintergrundbilder-Liste ausklappen"
          ...
          {showBackgroundImages && (
            <IconButton
              aria-label="Hintergrundbilder-Liste einklappen"
              onClick={() => setShowBackgroundImages(!showBackgroundImages)}
            >
              <ExpandLess />
            </IconButton>
      ...
      <Grid item container xs={12} justify="space-around">
        <Collapse
          in={showBackgroundImages}
          className={classes.fullWidthCollapse}
        >
          <Grid item xs={12}>
            <FormControlLabel
            ...
            <input
              ...
              disabled={
                ...
                !props.backGroundColorEnabled
              }
              value={
                !props.backGroundColorEnabled ? "#FFFFFF" : props.currentBGColor
              }
            />
            <br />
          </Grid>
          <Grid item container xs={12} justify="space-around">
            <Grid item>
              <Button
                ...
                onClick={props.handleBackgroundUploadClick}
                disabled={false}
              >
                HINTERGRUNDBILD HOCHLADEN
                <input
                  ref={props.backgroundUploadReference}
                  id={"backgroundUpload"}
                  type={"file"}
                  accept={".png, .jpg, .jpeg"}
                  hidden
          ...
          <Grid item container xs={12} className={classes.elementLargeMargin}>
            {props.backgroundImageList.map((image, index) =>
              renderImageEntry(image.image_blob_url, 0, "", index, "background")
            )}
          </Grid>
        </Collapse>
      </Grid>
      <Grid item container xs={12} className={classes.elementExtraLargeMargin}>
        ...
        <Grid item xs={2}>
          {!showImages && (
            <IconButton
              aria-label="Bilder-Liste ausklappen"
          ...
          {showImages && (
            <IconButton
              aria-label="Bilder-Liste einklappen"
              onClick={() => setShowImages(!showImages)}
            >
              <ExpandLess />
            </IconButton>
