...
export const StringProcessing: React.FC<StringProcessingProps> = (props) => {
  ...
  const [availableStrings, setAvailableStrings] = React.useState<Array<string>>(
   ...
   */
  const getAvailableStrings = React.useCallback(
    (listItems: Array<ListItemRepresentation>, noArray: boolean) => {
      ...
      listItems.forEach((listItem) => {
        if (noArray) {
          ...
          if (listItem.arrayRep) {
            ...
            availableStrings = availableStrings.concat(
              getAvailableStrings(listItem.value, false)
            );
          } else {
  ...
  const removeReplacementHandler = (index: number) => {
    ...
    if (
      props.historizedData.includes(props.stringReplacementList[index].name)
    ) {
      props.setHistorizedData(
        props.historizedData.filter((data) => {
  ...
  const renderProcessingsListEntry = (
    replacement: StringReplacementData,
    index: number
  ) => {
    return (
      ...
      <Grid container justify="space-between">
        <Grid item xs={12}>
          <Typography variant="body1">
            Eine String-Ersetzung kann erstellt werden, indem man einen String
            aus der Liste auswählt und festlegt, welche Zeichenketten auf welche
            Art und Weise ersetzt werden sollen.
          </Typography>
        ...
        >
          <Grid item xs={12}>
            <TextField
              ...
              variant="filled"
              color="primary"
              label={"Name der String-Ersetzung"}
              value={name}
          ...
          >
            <Grid item xs={12}>
              <Typography variant="h5">Strings</Typography>
            </Grid>
            <Grid item container xs={12} className={classes.elementLargeMargin}>
              ...
              >
                {availableStrings.map((string, index) =>
                  renderStringListItem(string, index)
                )}
              </Box>
            </Grid>
          </Grid>
          ...
          >
            <Grid item xs={12} style={{ height: "1%" }}>
              <Typography variant="h5">Ersetzen von:</Typography>
            </Grid>
            <Grid item xs={12}>
              <TextField
                fullWidth
                ...
                variant="filled"
                color="primary"
                label={"Zu ersetzender Substring"}
                value={replaceString}
                onChange={(e) => setReplaceString(e.target.value)}
              />
            </Grid>
            <Grid item xs={12} style={{ height: "1%" }}>
              <Typography variant="h5">Ersetzen mit:</Typography>
            </Grid>
            <Grid item xs={12}>
              <TextField
                fullWidth
                ...
                variant="filled"
                color="primary"
                label={"Einzusetzender String"}
                value={withString}
                onChange={(e) => setWithString(e.target.value)}
              />
            </Grid>
          </Grid>
          <Grid
          ...
          >
            <Grid item className={classes.blockableButtonSecondary}>
              <Button
                ...
                size="large"
                disabled={
                  name === "" || selectedStringIndex < 0 || replaceString === ""
                }
        ...
        <Grid item container xs={12} md={3}>
          ...
          >
            <Grid item container xs={12}>
              {props.stringReplacementList.map((processing, index) =>
