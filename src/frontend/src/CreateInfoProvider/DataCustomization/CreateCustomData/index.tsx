...
import { searchForComma } from "../../../EditInfoProvider/helpermethods";
  ...
  const handleSave = (formel: string) => {
    ...
    for (let i: number = 0; i <= props.customData.length - 1; i++) {
      if (props.customData[i].formelName === formel) {
        props.reportError(
          "Fehler: Name is schon an eine andere Formel vergeben!"
        );
        return;
      }
    }
      ...
      >
        <Grid item container xs={12}>
          <CustomDataGUI
            ...
            deleteCustomDataCheck={(formelName: string) =>
              deleteCustomDataCheck(formelName)
            }
      ...
      >
        ...
        <DialogContent dividers>
          ...
          {diagramsToRemove.length !== 0 && (
            <Typography gutterBottom className={classes.wrappedText}>
              Das Löschen der Formel wird folgende Diagramme entfernen, da sie
              die Formel nutzen: {diagramsToRemove.join(", ")}
            </Typography>
          )}
          {formulasToRemove.length !== 0 && (
            <Typography gutterBottom>
              Das Löschen der Formel wird folgende Formeln entfernen, da sie die
              Formel nutzen: {formulasToRemove.join(", ")}
              <br /> Durch das Löschen der Formeln können Diagramme und Formeln
              gelöscht werden, die diese nutzen (kaskadierende Löschung).
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
