...
export const ArrayProcessing: React.FC<ArrayProcessingProps> = (props) => {
  ...
  const [availableArrays, setAvailableArrays] = React.useState<
   ...
   */
  const getProcessableArrays = React.useCallback(
    (
      listItems: Array<ListItemRepresentation>,
      noArray: boolean,
      keyPath: string = "",
      ...
      listItems.forEach((listItem) => {
        ...
        if (noArray) {
          ...
          if (
            ...
            (listItem.value === "Zahl" || listItem.value === "Gleitkommazahl")
          )
            compatibleArraysList.push({
              ...
              innerKey:
                innerKeyPath === ""
                  ? listItem.keyName
                  : innerKeyPath + "|" + listItem.keyName, //this check should normally not be necessary since call with noArray === true will only be with a innerKeyPath
            });
            ...
            compatibleArraysList = compatibleArraysList.concat(
              getProcessableArrays(
                listItem.value,
                true,
                keyPath,
                innerKeyPath + "|" + listItem.keyName
              )
            );
          }
        } else {
          ...
          if (listItem.arrayRep) {
            ...
            if (!Array.isArray(listItem.value)) {
              if (
                ...
                compatibleArraysList.push({
                  valueInObject: false,
                  key: (listItem.parentKeyName === ""
                    ? listItem.keyName
                    : listItem.parentKeyName + "|" + listItem.keyName
                  ).replace("|0", ""),
                  innerKey: "",
                });
            } else {
            ...
            compatibleArraysList = compatibleArraysList.concat(
              getProcessableArrays(listItem.value, false)
            );
          }
        }
      });
  ...
  const checkDeleteDependencies = (name: string) => {
    ...
    props.customData.forEach((formula) => {
      if (formula.usedFormulaAndApiData.includes(name))
        formulasToRemove.current.push(formula.formelName);
    });
    ...
    return (
      formulasToRemove.current.length !== 0 ||
  ...
  const removeProcessing = (index: number) => {
    ...
    if (
      newHistorizedData.current.includes(props.arrayProcessingsList[index].name)
    ) {
  ...
  const deleteFormulaDependents = (formelName: string) => {
    ...
    newCustomData.current.forEach((formula) => {
      if (
        formula.usedFormulaAndApiData.includes(formelName + " ") ||
        formula.formelString.endsWith(formelName)
      )
        dependentFormulas.push(formula.formelName);
    });
  ...
  const renderArrayListItem = (array: ProcessableArray, index: number) => {
    ...
    const displayKey = array.valueInObject
      ? array.key + "|" + array.innerKey
      : array.key;
    return (
      <Grid item container xs={12} key={displayKey}>
        ...
        <Grid item xs={10}>
          <Typography
            variant="body1"
            className={classes.radioButtonListWrapText}
          >
            {displayKey}
          </Typography>
        </Grid>
   ...
   */
  const renderProcessingsListEntry = (
    processing: ArrayProcessingData,
    index: number
  ) => {
    return (
      ...
      <Grid container justify="space-between">
        <Grid item xs={12}>
          <Typography variant="body1">
            Erstellen sie Array-Verarbeitungen als neue Datenwerte, indem sie
            das gewünschte Array und die durchzuführende Operation auswählen:
          </Typography>
        </Grid>
        <Grid
          item
        ...
        >
          <Grid item xs={12}>
            <TextField
              ...
              variant="filled"
              color="primary"
              label={"Name der Array-Verarbeitung"}
              value={name}
          ...
          >
            <Grid item xs={12}>
              <Typography variant="h5">Arrays</Typography>
            </Grid>
            <Grid item container xs={12} className={classes.elementLargeMargin}>
              <Box
                ...
                border={4}
                borderRadius={5}
                className={classes.mediumListFrame}
              >
                {availableArrays.map((array, index) =>
                  renderArrayListItem(array, index)
                )}
              </Box>
            </Grid>
          </Grid>
          ...
          >
            <Grid item xs={12}>
              <Typography variant="h5">Operationen</Typography>
            </Grid>
            <Grid item xs={12} container className={classes.elementLargeMargin}>
              ...
              >
                {operations.map((operation, index) =>
                  renderOperationListItem(operation, index)
                )}
              </Box>
            </Grid>
          </Grid>
        ...
        <Grid item container xs={12} md={3}>
          ...
          >
            <Grid item container xs={12}>
              {props.arrayProcessingsList.map((processing, index) =>
                renderProcessingsListEntry(processing, index)
              )}
            </Grid>
          </Box>
        </Grid>
      ...
      >
        <DialogTitle id="deleteDialog-title" className={classes.wrappedText}>
          ...
          {currentRemoveIndex >= 0
            ? props.arrayProcessingsList[currentRemoveIndex].name
            : ""}
          " bestätigen
        </DialogTitle>
        <DialogContent dividers>
          <Typography gutterBottom className={classes.wrappedText}>
            Durch das Löschen von "
            {currentRemoveIndex >= 0
              ? props.arrayProcessingsList[currentRemoveIndex].name
              : ""}
            " müssen Formeln und/oder Diagramme gelöscht werden, die dieses
            nutzen.
          </Typography>
          <Typography gutterBottom className={classes.wrappedText}>
            {formulasToRemove.current.length > 0
              ? "Folgende Formeln sind betroffen: " +
                formulasToRemove.current.join(", ")
              : ""}
          </Typography>
          <Typography gutterBottom className={classes.wrappedText}>
            {diagramsToRemove.current.length > 0
              ? "Folgende Diagramme sind betroffen: " +
                diagramsToRemove.current.join(", ")
              : ""}
          </Typography>
        </DialogContent>
        <DialogActions>
