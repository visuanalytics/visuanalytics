...
export const CreateInfoProvider: React.FC<CreateInfoproviderProps> = (
  ...
  const steps =
    props.finishDataSourceInEdit === undefined
      ? [
          "Datenquellen-Typ",
          "API-Einstellungen",
          "Datenauswahl",
  ...
  const [historizedData, setHistorizedData] = React.useState(
    new Array<string>()
  );
  ...
  const [dataSourcesKeys, setDataSourcesKeys] = React.useState<
  ...
  const [arrayProcessingsList, setArrayProcessingsList] = React.useState<
    Array<ArrayProcessingData>
  >([]);
  //list of all string replacement processings defined
  const [stringReplacementList, setStringReplacementList] = React.useState<
    Array<StringReplacementData>
  >([]);
  ...
  const checkKeyExistence = React.useCallback(() => {
    ...
    for (let index = 0; index < dataSources.length; index++) {
      ...
      if (dataSourcesKeys.get(dataSource.apiName) !== undefined) {
        if (
          !dataSource.noKey &&
          (dataSourcesKeys.get(dataSource.apiName)!.apiKeyInput1 !== "" ||
            dataSourcesKeys.get(dataSource.apiName)!.apiKeyInput2 !== "")
        )
          return true;
      }
    }
  ...
  const authDialogNeeded = () => {
    const data: Array<DataSource> =
      sessionStorage.getItem("dataSources-" + uniqueId) === null
        ? new Array<DataSource>()
        : JSON.parse(sessionStorage.getItem("dataSources-" + uniqueId)!);
    const noKeyCurrent: boolean =
      sessionStorage.getItem("noKey-" + uniqueId) === "true";
    ...
    const methodCurrent: string =
      sessionStorage.getItem("method-" + uniqueId) || "";
  ...
  React.useEffect(() => {
    ...
    setCreateStep(
      Number(sessionStorage.getItem("createStep-" + uniqueId) || 0)
    );
    //name
    ...
    //selectedData
    setSelectedData(
      sessionStorage.getItem("selectedData-" + uniqueId) === null
        ? new Array<SelectedDataItem>()
        : JSON.parse(sessionStorage.getItem("selectedData-" + uniqueId)!)
    );
    //customData
    setCustomData(
      sessionStorage.getItem("customData-" + uniqueId) === null
        ? new Array<FormelObj>()
        : JSON.parse(sessionStorage.getItem("customData-" + uniqueId)!)
    );
    //historizedData
    setHistorizedData(
      sessionStorage.getItem("historizedData-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("historizedData-" + uniqueId)!)
    );
    //listItems (less calculations will be necessary this way)
    setListItems(
      sessionStorage.getItem("listItems-" + uniqueId) === null
        ? new Array<ListItemRepresentation>()
        : JSON.parse(sessionStorage.getItem("listItems-" + uniqueId)!)
    );
    //diagrams (less calculations will be necessary this way)
    setDiagrams(
      sessionStorage.getItem("diagrams-" + uniqueId) === null
        ? new Array<Diagram>()
        : JSON.parse(sessionStorage.getItem("diagrams-" + uniqueId)!)
    );
    //schedule
    setSchedule(
      sessionStorage.getItem("schedule-" + uniqueId) === null
        ? {
            type: "",
            interval: "",
            time: "",
    ...
    setHistorySelectionStep(
      Number(sessionStorage.getItem("historySelectionStep-" + uniqueId) || 1)
    );
    ...
    setArrayProcessingsList(
      sessionStorage.getItem("arrayProcessingsList-" + uniqueId) === null
        ? new Array<ArrayProcessingData>()
        : JSON.parse(
            sessionStorage.getItem("arrayProcessingsList-" + uniqueId)!
          )
    );
    //stringReplacementList
    setStringReplacementList(
      sessionStorage.getItem("stringReplacementList-" + uniqueId) === null
        ? new Array<StringReplacementData>()
        : JSON.parse(
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "historizedData-" + uniqueId,
      JSON.stringify(historizedData)
    );
  }, [historizedData]);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "arrayProcessingsList-" + uniqueId,
      JSON.stringify(arrayProcessingsList)
    );
  }, [arrayProcessingsList]);
      ...
      case 1:
        return (
          <BasicSettings
            ...
            checkNameDuplicate={
              props.checkDuplicateNameInEdit === undefined
                ? checkNameDuplicate
                : props.checkDuplicateNameInEdit
            }
            query={query}
            ...
            setListItems={(array: Array<ListItemRepresentation>) =>
              setListItems(array)
            }
            setArrayProcessingsList={(
      ...
      case 2:
        return (
          <DataSelection
            ...
            backHandler={handleBack}
            selectedData={selectedData}
            setSelectedData={(set: Array<SelectedDataItem>) =>
              setSelectedData(set)
            }
            listItems={listItems}
