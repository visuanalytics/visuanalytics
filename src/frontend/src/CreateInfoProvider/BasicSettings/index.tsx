...
export const BasicSettings: React.FC<BasicSettingsProps> = (props) => {
  ...
  React.useEffect(() => {
    if (
      sessionStorage.getItem("firstBasicSettingsEntering-" + uniqueId) !== null
    ) {
  ...
  const handleProceed = () => {
      ...
      diagramsCopy.forEach((diagram) => {
        if (
          ...
          for (let index = 0; index < diagram.arrayObjects.length; index++) {
            ...
            if (
              diagram.arrayObjects[index].listItem.parentKeyName.split(
                "|"
              )[0] === oldApiName
            ) {
              //overwrite the name for this array
              const parentKeyName =
                diagram.arrayObjects[index].listItem.parentKeyName;
              diagram.arrayObjects[index] = {
                ...
                listItem: {
                  ...
                  parentKeyName:
                    props.apiName +
                    diagram.arrayObjects[
                      index
                    ].listItem.parentKeyName.substring(
                      parentKeyName.split("|")[0].length,
          ...
          for (
            ...
            if (
              diagram.historizedObjects[index].name.split("|")[0] === oldApiName
            ) {
    ...
    const wasChanged = props.query !== oldQuery;
  ...
  const handleSuccess = (jsonData: any) => {
    ...
    if (data.status !== 0) {
      props.reportError(
        "Fehler: Backend meldet Fehler bei der API-Abfrage. Bitte überprüfen sie die Eingabe."
      );
  ...
  const handleError = (err: Error) => {
    props.reportError(
      "Fehler: Senden der Daten an das Backend fehlgeschlagen! (" +
        err.message +
        ")"
    );
    setDisplaySpinner(false);
        ...
        api_info: {
          ...
          api_key_name:
            props.method === "BearerToken"
              ? props.apiKeyInput1
              : props.apiKeyInput1 + "||" + props.apiKeyInput2,
  ...
  const handleMethodChange = (e: React.ChangeEvent<{ value: unknown }>) => {
    if (e.target !== null) {
      ...
      if (
        (methodIndex <= 1 && targetIndex > 1) ||
        (methodIndex === 2 && targetIndex !== 2) ||
        (methodIndex > 2 && targetIndex <= 2)
      ) {
          ...
          <form>
            <Grid container>
              ...
              {props.isInEditMode && (
                <Grid item xs={12}>
                  <Typography variant="body1">
                    Während der Bearbeitung von Infoprovidern ist die Änderung
                    des Namens der Datenquelle aus technischen Gründen nicht
                    möglich.
                  </Typography>
                </Grid>
              )}
              <Grid item xs={12} className={classes.elementSmallMargin}>
                <Typography variant="body1">
                  Bitte geben sie die Query an, die der Info-Provider nutzen
                  soll:
                </Typography>
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item container className={classes.additionalParams}>
                ...
                <Grid
                  item
                  className={`${classes.addParamButton}  ${classes.blockableButtonSecondary}`}
                >
                  <Button
              ...
              <Grid item container xs={12} justify="space-between">
                ...
                {props.method !== "BearerToken" && (
                  <Grid item xs={4}>
                    <APIInputField
                      defaultValue={
                        props.method === "BasicAuth" ? "Passwort" : "API-Key"
                      }
              ...
              <Grid item xs={12} className={classes.elementSmallMargin}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={props.noKey}
                      onChange={() => props.setNoKey(!props.noKey)}
                    />
                  }
              ...
              <Grid
                ...
                className={classes.elementSmallMargin}
              >
                <Grid item>
                  <Button
                    variant="contained"
                    ...
                    disabled={
                      !(
                        ...
                        (props.noKey ||
                          ...
                          (props.method === "BearerToken" &&
                            props.apiKeyInput1 !== "")) &&
                        !props.checkNameDuplicate(props.apiName)
                      )
                    }
                    variant="contained"
                    size="large"
      ...
      >
        ...
        <DialogContent dividers>
          <Typography gutterBottom>
            Durch eine Änderung in der API-Abfrage muss die API erneut angefragt
            werden.
          </Typography>
          <Typography gutterBottom>
            Durch die neue Abfrage und die mit ihr möglicherweise geänderten
            Antwortdaten müssen alle Formeln und Diagramme, die diese
            Datenquelle nutzen, entfernt werden, sofern die Abfrage erfolgreich
            ist.
          </Typography>
        </DialogContent>
        <DialogActions>
