...
export const SettingsOverview: React.FC<SettingsOverviewProps> = (props) => {
  ...
  const deleteDataSourceHandler = () => {
    ...
    props.diagrams.forEach((diagram) => {
      if (
        ...
        for (let index = 0; index < diagram.arrayObjects.length; index++) {
          if (
            diagram.arrayObjects[index].listItem.parentKeyName.split("|")[0] ===
            dataSourceName
          ) {
        ...
        for (let index = 0; index < diagram.historizedObjects.length; index++) {
          if (
            diagram.historizedObjects[index].name.split("|")[0] ===
            dataSourceName
          ) {
            localDiagramsToRemove.push(diagram.name);
            break;
    ...
    if (localDiagramsToRemove.length > 0)
      diagramsToRemove.current = localDiagramsToRemove;
  ...
  const deleteSelectedDataSource = (deleteDiagrams: boolean) => {
    ...
    props.setDataSources(
      props.dataSources.filter((dataSource) => {
        return (
          dataSource.apiName !== props.dataSources[selectedDataSource].apiName
        );
      })
    );
    ...
    if (
      mapCopy.get(props.dataSources[selectedDataSource].apiName) !== undefined
    ) {
  ...
  const manageBackDelete = () => {
    ...
    let dataSourceKeysTmp = props.dataSourcesKeys.get(
      props.dataSources[selectedDataSource].apiName
    );
   ...
   */
  const handleChangeSelectedDataSource = (
    ...
    if (props.dataSources[selectedDataSource].historizedData.length > 0) {
        ...
        <React.Fragment>
          <Grid item xs={12}>
            ...
            >
              <List disablePadding={true}>
                {props.dataSources[selectedDataSource].historizedData.map(
                  (item: string) => renderListItem(item)
                )}
              </List>
            </Box>
          </Grid>
          {props.dataSources[selectedDataSource].schedule.type !== "" && (
            <Grid item xs={12} className={classes.elementSmallMargin}>
              <Typography variant="h6">Historisierungszeiten</Typography>
            </Grid>
          )}
          <Grid item xs={12}>
            <Box
            ...
            >
              {props.dataSources[selectedDataSource].schedule.type !== "" && (
                <Grid item xs={12}>
                  <ScheduleTypeTable
                    schedule={props.dataSources[selectedDataSource].schedule}
                  />
                </Grid>
              )}
            </Box>
      ...
      return (
        <Typography variant="body1">
          Es wurden für diese Datenquelle keine Daten zur Historisierung
          ausgewählt.
        </Typography>
      );
    }
  ...
  const renderDataSource = (
    ...
    return (
      <MenuItem key={dataSource.apiName} value={dataSourceNumber}>
        {dataSource.apiName}
      </MenuItem>
    );
  };
    ...
    >
      <Grid container justify="space-between">
        <Grid item xs={12}>
          <TextField
            ...
            value={props.name}
            onChange={(event) =>
              props.setName(event.target.value.replace(" ", "_"))
            }
          ></TextField>
        </Grid>
        ...
        <Grid item container xs={12} md={6} justify="space-around">
          <Grid item xs={12}>
            <FormControl fullWidth>
              <InputLabel id="select-dataSource-dropDown">
                Wahl der Datenquelle
              </InputLabel>
              <Select
                value={selectedDataSource}
                onChange={handleChangeSelectedDataSource}
              >
                {props.dataSources.map((item: DataSource, index: number) =>
          ...
          >
            <Grid item className={classes.blockableButtonDelete}>
              <Button
                disabled={props.dataSources.length <= 1}
                variant="contained"
                size="large"
        ...
        >
          <Grid item xs={12}>
            <Typography variant="h5">Daten und Formeln</Typography>
          </Grid>
          <Grid item xs={12}>
            <Box
              borderColor="primary.main"
            ...
            >
              <List disablePadding={true}>
                {extractKeysFromSelection(
                  props.dataSources[selectedDataSource].selectedData
                ).map((item: string) => renderListItem(item))}
                {props.dataSources[selectedDataSource].customData.map(
                  (item: FormelObj) => renderListItem(item.formelName)
                )}
              </List>
            </Box>
          </Grid>
        ...
        <Grid
          ...
          xs={12}
          md={5}
          className={classes.historizedOverviewContainer}
        >
          <Grid item xs={12}>
            <Typography variant="h5">Zu historisierende Daten</Typography>
          </Grid>
          <Grid
            item
            container
            xs={12}
            className={
              props.dataSources[selectedDataSource].historizedData.length === 0
                ? classes.elementExtraLargeMargin
                : classes.elementLargeMargin
            }
          >
            {getHistoryContent()}
          </Grid>
        ...
        >
          ...
          <Grid item className={classes.blockableButtonSecondary}>
            <Button
              disabled={props.submitInfoProviderDisabled}
              variant="contained"
              size="large"
              color="secondary"
      ...
      >
        <DialogTitle id="deleteDialog-title" className={classes.wrappedText}>
          Löschen von "{props.dataSources[selectedDataSource].apiName}"
          bestätigen
        </DialogTitle>
        <DialogContent dividers>
          <Typography gutterBottom className={classes.wrappedText}>
            Die Datenquelle "{props.dataSources[selectedDataSource].apiName}"
            wird unwiderruflich gelöscht.
          </Typography>
          {diagramsToRemove.current.length > 0 && (
            <Typography gutterBottom className={classes.wrappedText}>
              Das Löschen der Datenquelle wird außerdem alle Diagramme löschen,
              die diese Datenquelle nutzen.
              <br />
              <br />
              Folgende Diagramme sind betroffen:{" "}
              <strong>{diagramsToRemove.current.join(", ")}</strong>
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
