...
export const ScheduleTypeTable: React.FC<ScheduleTypeTableProps> = (props) => {
  ...
  const generateTableRows = () => {
    ...
    if (props.schedule.type === "daily")
      rows.push(createTableRow("Uhrzeit", props.schedule.time!));
    if (props.schedule.type === "weekly")
      rows.push(
        createTableRow(
          "Ausgewählte Tage",
          getWeekdaySelectionString(props.schedule.weekdays)
        ),
        createTableRow("Uhrzeit", props.schedule.time!)
      );
    if (props.schedule.type === "interval")
      rows.push(
        createTableRow(
          "Gewähltes Intervall",
          getIntervalString(props.schedule.interval)!
        )
      );
    return rows;
  };
