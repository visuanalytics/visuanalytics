...
export const AuthDataDialog: React.FC<AuthDataDialogProps> = (props) => {
    ...
    for (let index = 0; index < props.selectionDataSources.length; index++) {
      ...
      if (element.name === "current--" + uniqueId) {
        ...
        if (props.method === "BearerToken") {
          ...
          if (props.apiKeyInput1 === "" || props.apiKeyInput2 === "")
            return false;
        }
      } else {
        ...
        if (keys !== undefined) {
          if (element.method === "BearerToken") {
            ...
            if (keys.apiKeyInput1 === "" || keys.apiKeyInput2 === "")
              return false;
          }
        }
      }
  ...
  /**
   * Handler for changes in the selected data source.
   * @param event The event causes by the change, containing the index of the new dataSource selected.
   */
  ...
  const handleInputChange = (field: number, value: string) => {
    if (
      props.selectionDataSources[selectedIndex] !== undefined &&
      value !== undefined
    ) {
      if (field === 1) {
        if (
          props.selectionDataSources[selectedIndex].name ===
          "current--" + uniqueId
        )
          props.setApiKeyInput1(value);
        else {
          const mapCopy = new Map(props.dataSourcesKeys);
          if (
            mapCopy.get(props.selectionDataSources[selectedIndex].name) !==
            undefined
          ) {
            mapCopy.set(props.selectionDataSources[selectedIndex].name, {
              apiKeyInput1: value,
              apiKeyInput2: mapCopy.get(
                props.selectionDataSources[selectedIndex].name
              )!.apiKeyInput2,
            });
  ...
  const renderDataSourceOption = (
    dataSource: string,
    dataSourceNumber: number
  ) => {
    if (dataSource !== undefined) {
      return (
        <MenuItem key={dataSource} value={dataSourceNumber}>
          {dataSource.includes(uniqueId)
  ...
  return (
    <Dialog
      aria-labelledby="authDataDialog-title"
    ...
    >
      ...
      <DialogContent dividers>
        <Grid container>
          <Grid item xs={12}>
            Erneute Eingabe der Authentifizierungsdaten ist auch
            Sicherheitsgründen notwendig.
            <br />
            Bitte geben sie für alle angezeigten Datenquellen ihre
            Authentifizierungsdaten erneut ein.
          </Grid>
          <Grid item xs={12} className={classes.elementLargeMargin}>
            <FormControl fullWidth>
              ...
              <Select value={selectedIndex} onChange={handleDataSourceChange}>
                {props.selectionDataSources.map((item, index) =>
                  renderDataSourceOption(item.name, index)
                )}
              </Select>
            </FormControl>
          </Grid>
          <Grid item xs={12} className={classes.elementLargeMargin}>
            <Typography variant="body1">
              Authentifizierungsmethode dieser Datenquelle:{" "}
              <strong>
                {resolveMethodName(
                  props.selectionDataSources[selectedIndex].method
                )}
              </strong>
            </Typography>
          </Grid>
      ...
      <DialogActions>
        <Grid item className={classes.blockableButtonPrimary}>
          <Button
            ...
            onClick={() => props.setAuthDataDialogOpen(false)}
          >
            Bestätigen
          </Button>
        </Grid>
