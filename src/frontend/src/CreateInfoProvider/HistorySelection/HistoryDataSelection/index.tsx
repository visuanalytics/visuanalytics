 ...
 */
export const HistoryDataSelection: React.FC<HistoryDataSelectionProps> = (
  props
) => {
  const classes = useStyles();

  //holds the selection on start to compare for diagram deletion detection
  const [oldHistorizedData, setOldHistorizedData] = React.useState(
  ...
  React.useEffect(() => {
    if (
      sessionStorage.getItem("firstHistDataSelectionEntering-" + uniqueId) !==
      null
    ) {
      setOldHistorizedData(
        sessionStorage.getItem("oldHistorizedData-" + uniqueId) === null
          ? []
          : JSON.parse(sessionStorage.getItem("oldHistorizedData-" + uniqueId)!)
      );
    } else {
      //leave a marker in the sessionStorage to identify if this is the first entering
      sessionStorage.setItem(
        "firstHistDataSelectionEntering-" + uniqueId,
        "false"
      );
    }
  }, []);
  React.useEffect(() => {
    sessionStorage.setItem(
      "oldHistorizedData-" + uniqueId,
      JSON.stringify(oldHistorizedData)
    );
  }, [oldHistorizedData]);
      ...
      props.diagrams.forEach((diagram) => {
        ...
        if (
          ...
          for (
            ...
            if (
              props.apiName + "|" + missingItem === historized.name &&
  ...
  const removeFromHistorySelection = (data: string) => {
    props.setHistorizedData(
      props.historizedData.filter((item) => {
        return item !== data;
      })
    );
  };
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Grid item xs={10}>
          ...
          >
            <List disablePadding={true} key="listRoot2">
              ...
              {props.arrayProcessingsList.map((item) =>
                renderListItem(item.name)
              )}
              {props.stringReplacementList.map((item) =>
                renderListItem(item.name)
              )}
            </List>
          </Box>
        ...
        >
          ...
          <Grid item>
            ...
            >
              {props.historizedData.length === 0 &&
              props.newDataSourceInEditMode
                ? "abschließen"
                : "weiter"}
            </Button>
          </Grid>
      ...
      >
        ...
        <DialogContent dividers>
          <Typography gutterBottom>
            Durch das Abwählen einiger Daten müssen Diagramme gelöscht werden,
