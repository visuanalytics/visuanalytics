 ...
 */
export const extractKeysFromSelection = (
    ...
    if (stringRep[0] === "{") {
      ...
      if (nextKey === "same_type") {
        ...
        if (array_length === "0") {
          ...
          let element = subObject
            .substring(24 + same_type_value.length + array_length.length)
            .split(":", 1)[0];
          if (element.substring(1, element.length - 1) === "object") {
            ...
            let object = subObject.substring(
              33 + same_type_value.length + array_length.length,
              subObject.length - 1
            );
            let objectLookahead = object.split(":")[0];
            if (
              objectLookahead.substring(2, objectLookahead.length - 1) ===
              "same_type"
            ) {
              value = "[Array]";
            } else {
            ...
            value = subObject.substring(
              32 + same_type_value.length + array_length.length,
              subObject.length - 2
            );
            resultArray.push({
              keyName: key + "|0",
          ...
          let object = subObject.substring(
            31 + same_type_value.length + array_length.length,
            subObject.length - 1
          );
          let typeString = "";
          ...
          if (typeString.includes("{") && typeString.includes("}"))
            typeString = "different object types";
          resultArray.push({
            keyName: key + "|0",
            value: typeString,
            parentKeyName: parent,
...
export const transformBackendInfoProvider = (data: InfoProviderFromBackend) => {
  ...
  data.datasources.forEach((backendDataSource) => {
    ...
    dataSources.push({
      ...
      method:
        backendDataSource.api.method === "noAuth"
          ? ""
          : backendDataSource.api.method,
    ...
    if (backendDataSource.api.method === "BearerToken") {
      ...
      apiKeyInput2 = backendDataSource.api.api_info.api_key_name.substring(
        apiKeyInput1.length + 2
      );
    }
