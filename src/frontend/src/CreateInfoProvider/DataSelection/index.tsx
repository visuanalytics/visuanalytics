...
export const DataSelection: React.FC<DataSelectionProps> = (props) => {
  ...
  const [oldSelectedData, setOldSelectedData] = React.useState(
    props.selectedData
  );
  ...
  const [historizedToRemove, setHistorizedToRemove] = React.useState<
    Array<string>
  >([]);
  ...
  React.useEffect(() => {
    if (
      sessionStorage.getItem("firstDataSelectionEntering-" + uniqueId) !== null
    ) {
      setOldSelectedData(
        sessionStorage.getItem("oldSelectedData-" + uniqueId) === null
          ? []
          : JSON.parse(sessionStorage.getItem("oldSelectedData-" + uniqueId)!)
      );
    } else {
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "oldSelectedData-" + uniqueId,
      JSON.stringify(oldSelectedData)
    );
  }, [oldSelectedData]);
  ...
  const getFormulasUsingData = (data: string) => {
    ...
    props.customData.forEach((formula) => {
      ...
      if (formula.usedFormulaAndApiData.includes(data))
        formulas.push(formula.formelName);
    });
    return formulas;
  };
  ...
  const calculateItemsToRemove = () => {
    ...
    if (missingSelections.length > 0) {
      ...
      removalObj.historizedToRemove = props.historizedData.filter((item) => {
        return (
          missingSelections.includes(item) ||
          removalObj.formulasToRemove.includes(item)
        );
      });
        ...
        props.diagrams.forEach((diagram) => {
          ...
          if (
            ...
            for (
              ...
              if (
                props.apiName + "|" + formula === historized.name &&
        ...
        props.diagrams.forEach((diagram) => {
          if (
            ...
            for (
              ...
              if (
                props.apiName + "|" + historizedData === historized.name &&
                !diagramsToRemove.includes(diagram.name)
              ) {
                diagramsToRemove.push(diagram.name);
                break;
  ...
  const handleContinue = () => {
    ...
    if (
      removalObj.formulasToRemove.length > 0 ||
      removalObj.diagramsToRemove.length > 0
    ) {
    ...
    if (Array.isArray(data.value)) {
      ...
      return (
        <Grid
          ...
          container
          justify="flex-end"
          xs={xsSize as GridSize}
          className={classes.dataSelectionListItem}
          key={
            data.parentKeyName === ""
              ? data.keyName
              : data.parentKeyName + "|" + data.keyName
          }
        >
          <Grid item container xs={12}>
            <Grid item xs={12}>
              <Typography className={classes.processingListingText}>
                {data.arrayRep
                  ? data.keyName + " (Array[0]), length: " + data.arrayLength
                  : data.keyName + " (object)"}
              </Typography>
            </Grid>
          </Grid>
        ...
        >
          <Grid item className={classes.dataSelectionCheckboxItem}>
            {!(
              data.value === "[Array]" ||
              data.value.includes(", ") ||
              data.value === "different object types"
            ) && (
              <FormControlLabel
                control={
                  <Checkbox
                    onClick={() => checkboxHandler(selectedDataObj)}
                    checked={extractKeysFromSelection(
                      props.selectedData
                    ).includes(selectedDataObj.key)}
                  />
                }
                label={""}
              />
  ...
  const removeFromSelection = (data: SelectedDataItem) => {
    props.setSelectedData(
      props.selectedData.filter((item) => {
  ...
  return (
    <StepFrame heading="Datenauswahl" hintContent={hintContents.dataSelection}>
      <Grid
        container
      ...
      >
        ...
        <Grid item xs={12} className={classes.elementLargeMargin}>
          <Typography variant="body2">
            Die Daten sehen fehlerhaft aus? Gehen sie einen Schritt zurück und
            prüfen sie Request und Key der API.
          </Typography>
        </Grid>
        <Grid
          item
      ...
      >
        ...
        <DialogContent dividers>
          <Typography gutterBottom>
            Das Zurückgehen zum vorherigen Schritt erfordert, dass alle neu
            gewählten oder abgewählten Daten verworfen werden.
          </Typography>
          <Typography gutterBottom>Wirklich zurückgehen?</Typography>
        </DialogContent>
        <DialogActions>
