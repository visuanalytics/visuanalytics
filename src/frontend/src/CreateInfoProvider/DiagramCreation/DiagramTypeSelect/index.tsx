...
export const DiagramTypeSelect: React.FC<DiagramTypeSelectProps> = (props) => {
  ...
  const [selectedHistorized, setSelectedHistorized] = React.useState<
    Array<string>
  >([]);
  ...
  React.useEffect(() => {
    ...
    //selectedArrays
    setSelectedArrays(
      sessionStorage.getItem("selectedArrays-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("selectedArrays-" + uniqueId)!)
    );
    //selectedHistorized
    setSelectedHistorized(
      sessionStorage.getItem("selectedHistorized-" + uniqueId) === null
        ? new Array<string>()
        : JSON.parse(sessionStorage.getItem("selectedHistorized-" + uniqueId)!)
    );
  }, []);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "selectedArrays-" + uniqueId,
      JSON.stringify(selectedArrays)
    );
  }, [selectedArrays]);
  //store selectedHistorized in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "selectedHistorized-" + uniqueId,
      JSON.stringify(selectedHistorized)
    );
  }, [selectedHistorized]);
  ...
  const getNumericAttributes = (
    object: Array<ListItemRepresentation>,
    ...
    if (baseArrayName === "")
      baseArrayName = object.length > 0 ? object[0].parentKeyName : "";
    ...
    for (let index = 0; index < object.length; ++index) {
      ...
      if (
        ...
        const keyPath = (
          object[index].parentKeyName +
          "|" +
          object[index].keyName
        ).substring(baseArrayName.length + 1);
        //console.log(keyPath);
      ...
      //check if this is an sub-object - we need to check it too then
        ...
        numericAttributes = numericAttributes.concat(
          getNumericAttributes(
            object[index].value as Array<ListItemRepresentation>,
            baseArrayName
          )
        );
      }
  ...
  const getStringAttributes = (object: Array<ListItemRepresentation>) => {
    ...
    for (let index = 0; index < object.length; ++index) {
      ...
      //check if this is an sub-object - we need to check it too then
        ...
        stringAttributes = stringAttributes.concat(
          getStringAttributes(
            object[index].value as Array<ListItemRepresentation>
          )
        );
  ...
  const createArrayObjects = () => {
    ...
    selectedArrays.forEach((array) => {
      ...
      for (let index = 0; index < props.compatibleArrays.length; ++index) {
        ...
        if (
          (element.parentKeyName === ""
            ? element.keyName
            : element.parentKeyName + "|" + element.keyName) === array
        ) {
          item = element;
          break;
        }
      ...
      if (Object.keys(item).length !== 0) {
        const numericAttributes = Array.isArray(item.value)
          ? getNumericAttributes(item.value)
          : [];
        const stringAttributes = Array.isArray(item.value)
          ? getStringAttributes(item.value)
          : [];
        const arrayObject = {
          listItem: item,
          numericAttribute: "",
  ...
  const removeFromHistorizedSelection = (data: string) => {
    setSelectedHistorized(
      selectedHistorized.filter((item) => {
        return item !== data;
      })
    );
  };
      ...
      <ListItem key={keyString}>
        <ListItemIcon>
          <FormControlLabel
            control={
              <Checkbox
                onClick={() => arrayCheckboxHandler(keyString)}
      ...
      <ListItem key={item}>
        <ListItemIcon>
          <FormControlLabel
            control={
              <Checkbox
                onClick={() => historizedCheckboxHandler(item)}
    ...
    <Grid container>
      <Grid item xs={12}>
        <Typography variant="body1">
          Bitte wählen sie aus, ob das Diagramm aus einem Array oder
          historisierten Daten erstellt werden soll:
        </Typography>
      </Grid>
            ...
            <Grid item xs={12}>
              ...
              <Collapse in={selectedType === "Array"}>
                <Grid item container xs={12}>
                  <Grid item xs={12}>
                    <Typography variant="body1">
                      Folgende Arrays des Infoproviders sind kompatibel mit
                      Diagrammen:
                    </Typography>
                  </Grid>
                  <Grid item container xs={12}>
                    ...
                    >
                      <Grid item xs={12}>
                        <List disablePadding={true}>
                          {props.compatibleArrays.map((item) =>
            ...
            <Grid item xs={12}>
              ...
              <Collapse in={selectedType === "HistorizedData"}>
                <Grid item container xs={12}>
                  <Grid item xs={12}>
                    <Typography variant="body1">
                      Folgende historisierte Daten sind für die
                      Diagrammerstellung geeignet:
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Box
                    ...
                    >
                      <List disablePadding={true}>
                        {props.compatibleHistorized.map((item) =>
                          renderHistorizedListItem(item)
                        )}
                      </List>
                    </Box>
                  </Grid>
      ...
      >
        ...
        <Grid item className={classes.blockableButtonPrimary}>
          <Button
            disabled={
              !(
                (selectedType === "Array" && selectedArrays.length !== 0) ||
                (selectedType === "HistorizedData" &&
                  selectedHistorized.length !== 0)
              )
            }
            variant="contained"
            size="large"
