 ...
 */
export const ArrayDiagramCreator: React.FC<ArrayDiagramCreatorProps> = (
  props
) => {
  ...
  const [selectedArrayOrdinal, setSelectedArrayOrdinal] =
    React.useState<number>(0);
  //holds a list of all available string attributes
  const [availableStringAttributes, setAvailableStringAttributes] =
    React.useState<Array<SelectedStringAttribute>>([]);
  ...
  React.useEffect(() => {
    ...
    setSelectedArrayOrdinal(
      Number(sessionStorage.getItem("selectedArrayOrdinal-" + uniqueId) || 0)
    );
    //availableStringAttributes
    setAvailableStringAttributes(
      sessionStorage.getItem("availableStringAttributes-" + uniqueId) === null
        ? []
        : JSON.parse(
            sessionStorage.getItem("availableStringAttributes-" + uniqueId)!
          )
    );
  }, []);
  ...
  React.useEffect(() => {
    sessionStorage.setItem(
      "selectedArrayOrdinal-" + uniqueId,
      selectedArrayOrdinal.toString()
    );
  }, [selectedArrayOrdinal]);
  //store availableStringAttributes in sessionStorage
  React.useEffect(() => {
    sessionStorage.setItem(
      "availableStringAttributes-" + uniqueId,
      JSON.stringify(availableStringAttributes)
    );
  }, [availableStringAttributes]);
  ...
  React.useEffect(() => {
    ...
    arrayObjects.forEach((array) => {
      array.stringAttributes.forEach((stringAttribute) => {
        ...
        const arrayFull =
          array.listItem.parentKeyName === ""
            ? array.listItem.keyName
            : array.listItem.parentKeyName + "|" + array.listItem.keyName;
        const arrayNoDataSource =
          arrayFull.substring(arrayFull.indexOf("|") + 1) + "|0|";
        //now get the innerKey without the parent to use it for generating
        const innerKeyFull =
          stringAttribute.parentKeyName + "|" + stringAttribute.keyName;
        const innerKeyNoParent = innerKeyFull.substring(
          arrayNoDataSource.length
        );
  ...
  const checkProceed = () => {
    ...
    if (props.customLabels) {
      ...
      if (
        props.selectedStringAttribute.key === "" ||
        props.selectedStringAttribute.array === ""
      )
        return false;
    }
    ...
    for (let i = 0; i < props.arrayObjects.length; i++) {
      ...
      if (Array.isArray(item.listItem.value) && item.numericAttribute === "")
        return false;
    }
    return true;
  };
   ...
   */
  const selectedArrayChangeHandler = (
   ...
   */
  const renderArraySelectItem = (
    object: ArrayDiagramProperties,
    index: number
  ) => {
    const keyString =
      object.listItem.parentKeyName === ""
        ? object.listItem.keyName
        : object.listItem.parentKeyName + "|" + object.listItem.keyName;
    return (
   ...
   */
  const numericAttributeChangeHandler = (
   ...
   * @param event The change event caused by changing the attribute.
   */
  const stringAttributeChangeHandler = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const renderNumericAttributeItem = (item: ListItemRepresentation) => {
    return (
      <FormControlLabel
        className={classes.wrappedLabel}
        value={item.keyName}
        control={<Radio />}
        label={item.keyName}
        key={item.keyName}
   ...
   */
  const renderStringAttributeItem = (
    item: SelectedStringAttribute,
    index: number
  ) => {
    return (
      <FormControlLabel
        ...
        value={index}
        control={<Radio />}
        label={item.array + "|" + item.key}
        key={item.array + "|" + item.key}
      />
    );
  };
      ...
      <Grid item container xs={12} className={classes.elementLargeMargin}>
        <Grid item xs={6}>
          <Typography variant="body1">
            {Array.isArray(
              props.arrayObjects[selectedArrayOrdinal].listItem.value
            )
              ? "Bitte wählen sie das Zahl-Attribut zur Darstellung im Diagramm:"
              : "Das Array enthält primitive Werte, eine Attribut-Auswahl ist nicht notwendig."}
          </Typography>
        </Grid>
        <Grid item xs={6}>
          <Typography variant="body1">
            {props.customLabels
              ...
              : "Bitte wählen sie das String-Attribut zur Beschriftung der Werte:"}
          </Typography>
        </Grid>
        <Grid item xs={6}>
          {Array.isArray(
            ...
            >
              <FormControl>
                <RadioGroup
                  value={
                    props.arrayObjects[selectedArrayOrdinal].numericAttribute
                  }
                  onChange={numericAttributeChangeHandler}
                >
                  {props.arrayObjects[
                    selectedArrayOrdinal
                  ].numericAttributes.map((item) =>
                    renderNumericAttributeItem(item)
                  )}
                </RadioGroup>
              </FormControl>
            </Box>
      ...
      return (
        <Grid item xs={6}>
          ...
          >
            <FormControl>
              <RadioGroup
                value={stringAttributeIndex}
                onChange={stringAttributeChangeHandler}
              >
                {availableStringAttributes.map((item, index) =>
                  renderStringAttributeItem(item, index)
                )}
                {availableStringAttributes.length === 0 && (
                  <Typography variant="body1">
                    Die Objekte der gewählten Arrays enthalten keine
                    String-Attribute.
                  </Typography>
                )}
              </RadioGroup>
    ...
    <Grid container justify="space-between">
      ...
      <Grid item xs={8} className={classes.elementLargeMargin}>
        <FormControl fullWidth variant="outlined">
          ...
          >
            {props.arrayObjects.map((object, index) =>
              renderArraySelectItem(object, index)
            )}
          </Select>
        </FormControl>
      </Grid>
      ...
      >
        <Grid item xs={6}>
          <Typography variant="body1">Farbe im Diagramm:</Typography>
        </Grid>
        <Grid item xs={6}>
          <input
            type="color"
      ...
      <Dialog
        ...
        aria-labelledby="leaveDialog-title"
        open={cancelOpen}
      >
