...
interface HistorizedDiagramCreatorProps {
  ...
  changeObjectInHistorizedObjects: (
 ...
 */
export const HistorizedDiagramCreator: React.FC<HistorizedDiagramCreatorProps> =
  (props) => {
    ...
    const [selectedHistorizedOrdinal, setSelectedHistorizedOrdinal] =
    ...
    React.useEffect(() => {
      ...
      setSelectedHistorizedOrdinal(
        Number(
          sessionStorage.getItem("selectedHistorizedOrdinal-" + uniqueId) || 0
        )
      );
    }, []);
    ...
    React.useEffect(() => {
      sessionStorage.setItem(
        "selectedHistorizedOrdinal-" + uniqueId,
        selectedHistorizedOrdinal.toString()
      );
    }, [selectedHistorizedOrdinal]);
    ...
    const delayedColorChange = (color: string) => {
      ...
      timeOut = window.setTimeout(() => {
        ...
        props.changeObjectInHistorizedObjects(
          objCopy,
          selectedHistorizedOrdinal
        );
      }, 200);
     ...
     */
    const selectedHistorizedChangeHandler = (
     ...
     */
    const renderHistorizedSelectItem = (
      object: HistorizedDiagramProperties,
      index: number
    ) => {
      return (
     ...
     */
    const intervalSizeChangeHandler = (
      event: React.ChangeEvent<HTMLInputElement>,
      ordinal: number
    ) => {
      const newIntervalSizes = [
        ...props.historizedObjects[selectedHistorizedOrdinal].intervalSizes,
      ];
    ...
    const getIntervalDisplay = () => {
      ...
      const currentDataSourceName =
        props.historizedObjects[selectedHistorizedOrdinal].name.split("|")[0];
        ...
        <ListItem key={"Intervallgröße_" + (ordinal + 1)} divider={true}>
          <Grid item container xs={12} justify="space-between">
              ...
              <FormControlLabel
                ...
                control={
                  <TextField
                    ...
                    value={
                      props.historizedObjects[selectedHistorizedOrdinal]
                        .intervalSizes[ordinal]
                    }
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      intervalSizeChangeHandler(e, ordinal)
                    }
            ...
            <Grid item xs={2} className={classes.intervalChoiceRightLabel}>
              <Typography variant="body1">Intervalle</Typography>
            </Grid>
          </Grid>
        </ListItem>
      );
            ...
            >
              <List>
                {Array.from(Array(props.amount).keys()).map((ordinal) =>
                  renderIntervallChoiceItem(ordinal)
                )}
              </List>
            </Box>
          </Grid>
      ...
      <Grid container justify="space-between">
        ...
        <Grid item xs={8} className={classes.elementLargeMargin}>
          <FormControl fullWidth variant="outlined">
            ...
            >
              {props.historizedObjects.map((object, index) =>
                renderHistorizedSelectItem(object, index)
              )}
            </Select>
          </FormControl>
        </Grid>
        ...
        <Grid item xs={12} className={classes.elementLargeMargin}>
          <Typography>
            Dieser Wert verwendet eine Intervallgröße von:{" "}
            {getIntervalDisplay()}
          </Typography>
        </Grid>
        {renderSelections()}
