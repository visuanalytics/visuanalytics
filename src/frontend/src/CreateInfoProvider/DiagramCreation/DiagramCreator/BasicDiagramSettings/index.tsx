 ...
 */
export const BasicDiagramSettings: React.FC<BasicDiagramSettingsProps> = (
  props
) => {
  ...
  const evaluateAmount = () => {
    if (props.arrayObjects !== undefined) {
      for (let index = 0; index < props.arrayObjects!.length; index++) {
        if (props.amount > props.arrayObjects![index].listItem.arrayLength)
          return true;
      }
      return false;
    }
   ...
   */
  const diagramTypeChangeHandler = (
  ...
  return (
    <React.Fragment>
              ...
              <MenuItem
                ...
                disabled={
                  (props.arrayObjects !== undefined &&
                    props.arrayObjects.length > 1) ||
                  (props.historizedObjects !== undefined &&
                    props.historizedObjects.length > 1)
                }
              >
                Tortendiagramm (nur bei max. einem{" "}
                {props.arrayObjects !== undefined
                  ? "Array"
                  : "historisiertes Datum"}
                )
              </MenuItem>
      ...
      <Grid
        ...
        container
        xs={12}
        justify={props.arrayObjects !== undefined ? "flex-start" : "flex-start"}
      >
        {props.arrayObjects !== undefined && (
          <Grid item xs={7} className={classes.amountWarningContainer}>
            {evaluateAmount() && (
              <Alert severity="warning">
                <strong>Warnung:</strong> Die gewählte Anzahl überschreitet die
                Größe der Testdaten von mindestens einem Array.
              </Alert>
            )}
          </Grid>
        )}
