...
export const DiagramOverview: React.FC<DiagramOverviewProps> = (props) => {
   ...
   */
  const handleSuccessDiagramPreview = React.useCallback(
    (jsonData: any) => {
      ...
      <ListItem key={item.name} divider={true}>
        <Grid item xs={9}>
          <Typography className={classes.wrappedText}>
            {item.name +
              " (" +
              resolveType(item.variant) +
              ") - nutzt " +
              (item.sourceType === "Array" ? "Arrays" : "historisierte Daten")}
          </Typography>
        </Grid>
        <Grid item xs={3}>
          <ListItemSecondaryAction>
            <IconButton
              ...
              color="primary"
              onClick={() => openPreviewDialog(item.name)}
            >
              <ImageIcon />
            </IconButton>
            <IconButton
    ...
    <Grid container justify="space-around">
      ...
      <Grid item>
        <Button
          ...
          size="large"
          color="secondary"
          onClick={props.createDiagramHandler}
        >
          Diagramm hinzufügen
        </Button>
      </Grid>
