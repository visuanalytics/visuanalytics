...
export const validateParamValue = (value: any, param: Param) => {
  ...
  switch (param.type) {
    ...
    case "multiNumber":
      ...
      if (!optional) {
        return (
          mnv.length > 0 && mnv.every((v) => v !== "" && !isNaN(Number(v)))
        );
      }
      return (
        mnv.filter((v) => v !== "").length === 0 ||
        mnv.every((v) => v !== "" && !isNaN(Number(v)))
      );
    default:
      return true;
  }
...
export const getInvalidParamValues = (
  values: ParamValues,
  params: Param[] | undefined
): string[] => {
  if (
    params === undefined ||
    (params.length > 0 && Object.keys(values).length === 0)
  )
    return [""];
...
export const initSelectedValues = (params: Param[] | undefined) => {
  ...
  params?.forEach((p) => {
    switch (p.type) {
      case "string":
        selected[p.name] =
          p.optional && p.defaultValue !== undefined ? p.defaultValue : "";
        break;
      case "number":
        selected[p.name] =
          p.optional && p.defaultValue !== undefined ? p.defaultValue : "";
      ...
      case "boolean":
        selected[p.name] =
          p.optional && p.defaultValue !== undefined ? p.defaultValue : false;
        break;
      case "subParams":
        if (p.optional) {
