...
export const fromFormattedDates = (fSchedule: any): Schedule => {
  ...
  if (fSchedule.type === "interval") {
    ...
    if (fSchedule.nextExecution === null) {
      ...
      let nextExecutionStr = String(fSchedule.nextExecution)
        .replace(" ", "-")
        .substr(0, 19);
      nextExecution = parse(
        nextExecutionStr,
...
export const showSchedule = (schedule: Schedule) => {
  switch (schedule.type) {
    case "daily":
      return (
        "täglich, " + format(schedule.time, "HH:mm", { locale: de }) + " Uhr"
      );
    case "onDate":
      return (
        format(schedule.date, "dd.MM.yyyy") +
        ", " +
        format(schedule.time, "HH:mm", { locale: de }) +
        " Uhr"
      );
    case "weekly":
