...
export const JobSettings: React.FC<Props> = ({
  ...
  const handleCheckClick = () => {
    ...
    const invalid = job.topicValues.map((t: any, idx: number) =>
      getInvalidParamValues(paramValues[idx], t.params)
    );
    setInvalidValues(invalid ? invalid : []);
    const paramsValid = invalid?.every((t) => t.length === 0);
    if (!paramsValid) {
      errorMessage =
        errorMessage !== "" ? errorMessage : "Parameter nicht korrekt gesetzt";
    } else {
      setInvalidValues([]);
    }
